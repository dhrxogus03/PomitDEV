...
<%!
static public boolean resumeRoute(Context context, String sRouteId, boolean flag, boolean isSubRoute) throws Exception
{
  ...
  if(nResult!=0){
  	...
  	return false;
		...
		throw new Exception("Route resume failed");
  }
...
/* 	MQLCommand command = new MQLCommand();
    ...
    while(it.hasNext())
    {
        ...
        if(routeId!=null)
        {
        	resumeRoute(context, routeId, true, true);
        }
    }
	return true;	
}
  ...
  if (bIsRouteStartable)
  {
    for(int count=0; count < routeIds.length; count++)
    {
      ...
      if ((fromPage != null) && (!fromPage.equals("routeProperties")))
      {
       ...
       try
       {
          ...
          else if("Stopped".equals(routeStatus))
          {
              ...
              if (Route.isRouteStoppedDueToRejection(context, routeIds[count])) {
            	  ...
            	  resumeSuccessful = resumeRoute(context, routeIds[count], false, false);
            }
              if(resumeSuccessful){
       ...
       {
    	   boolean clientTaskMessagesExists = MqlNoticeUtil.checkIfClientTaskMessageExists(context);
		   ...
		   while (itrNotices.next()) {
				...
				if(emxMessage.contains("EKLProgramJPO triggered")){
					emxMessage = Route.parseExceptionPrecommit(emxMessage);
					context.clearClientTasks();
    				MqlUtil.mqlCommand(context, "notice $1",emxMessage);
				}
		   }
