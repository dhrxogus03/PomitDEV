  ...
  { 
	...
	<%  if("Yes".equals(strUserGroup)) {
	    if(UINavigatorUtil.isCloud(context)){%>
		  var sURL="../common/emxFullSearch.jsp?field=TYPES=type_Group:CURRENT=policy_Person.state_Active&table=AEFUserGroupsChooser&form=AEFSearchUserGroupsForm&selection=multiple&showSavedQuery=True&source=usersgroup&rdfQuery=[ds6w:type]:(Group)&submitURL=";
		<%}else{%>
		 var sURL="../common/emxFullSearch.jsp?field=TYPES=type_Group:CURRENT=policy_Person.state_Active&table=AEFUserGroupsChooser&form=AEFSearchUserGroupsForm&selection=multiple&showSavedQuery=True&submitURL=";
		<%}%>
		<% }else{ %>
