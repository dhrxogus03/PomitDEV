...
String nlsValueNotEqual = DAuUtilities.getSpecializationStringResource(context,"emxDataSpecialization.NotEqualNLSValue");  
%>
...
var nlsValueNotEqual = "<%=nlsValueNotEqual%>";
...
function onClickSaveManageNLSTable(){
	// check row values 
	var error = ["<mxRoot>"];
	var rRows = getNodes(oXML, "/mxRoot/rows//r[@status='add' or @status='changed']");
	var validated = true;
	for(var i = 0; i < rRows.length; i++)
	{
		if(!validateRow(rRows[i]))
		{
			validated = false;
			error.push("<object rowId=\"" + rRows[i].id + "\">")
			error.push("<error>" + nlsValueNotEqual + "</error></object>");
...
function validateRow(row) {
	...
	{
		if(childNodes[i].textContent.contains("=")){
			validated = false;
			break;
		}
	}
...
function onClickCancelManageNLSTable(){
	switchToViewMode(true, true);
}
