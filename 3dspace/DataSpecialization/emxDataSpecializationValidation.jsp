...
<%@page import = "com.dassault_systemes.DictionaryAuthorModelUI.HasDefaultField"%>
...
String nlsIconNameNotAlphaNum = DAuUtilities.getSpecializationStringResource(context,"emxDataSpecialization.NotAlphanumericIconName");
...
boolean accessNewHasDefaultEditField = HasDefaultField.accessHasDefaultFieldEditMode(context);
...
function checkNames(){
	...
	//Check Type Name
        ...
        //Check Icon Name (if iconName exists and is not blank)
        if (undefined != document.forms['editDataForm'].elements['IconName']  &&
           document.forms['editDataForm'].elements['IconName'].value != ""){ 
            if(! re.test(trim(document.forms['editDataForm'].elements['IconName'].value))) {
                alert("<%=nlsIconNameNotAlphaNum%>");
                return false;
            }
        }
	 }
...
function checkDefaultValue() {
	...
	// IR-999016-3DEXPERIENCER2022x : skip the check if there is no default value
	if(<%=accessNewHasDefaultEditField%> && document.forms['editDataForm'].elements['hasDefaultValField'] && document.forms['editDataForm'].elements['hasDefaultValField'].value == 'false') {
		return true;
	}
...
//S63 IR-999016-3DEXPERIENCER2022x Now using formValidation.js
function onChangeMultiValueDeprecated(input){
...
//S63 IR-999016-3DEXPERIENCER2022x Now using formValidation.js
function onChangeEditMultiValueDeprecated(input){
	// BMN2-IR-347926 03/02/15
	/*if(input.options[input.selectedIndex].value=='true'){
		setDisableRangeFields(true);
		removeEditRangeFields();
