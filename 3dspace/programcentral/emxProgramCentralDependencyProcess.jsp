        ...
        for (int i=0; numTasks>i; i++) {
          if(isWin9x) {
           ...
           lagTime        = (String) emxGetParameter(request, "lag_" + connectionIds[i]);
			...
			else if(unitTime.equals("M") && ProgramCentralUtil.isNotNullString(dependencyType)){
			lagTime = task.getDependencyInDays(context, dependencyType,lagTime,tasks[i]);
			unitTime = " d";
			}
 ...
 //End:R209:PRG :Keyword Duration
          ...
          while (predecessorItr.hasNext()) {
            ...
            if(connectionIds[i].equals(predecessorObj.get(dependency.SELECT_ID))) {
              ...
              if(unitTime.equals("M") && ProgramCentralUtil.isNullString(dependencyType)){
                dependencyType = (String) predecessorObj.get(dependency.SELECT_DEPENDENCY_TYPE);
      			lagTime = task.getDependencyInDays(context, dependencyType,lagTime,tasks[i]);
      			unitTime = " d";
    			sAttributeValue=lagTime+unitTime;
      			}
