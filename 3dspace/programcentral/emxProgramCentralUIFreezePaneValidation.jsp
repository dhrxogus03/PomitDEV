...
<%
	...
	String numeric = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.PassiveSubtask.WarningLevel.NumericVal",strLanguage);
	String positiveWholeVal = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.PassiveSubtask.WarningLevel.PositiveWholeVal",strLanguage);
	String positiveWholeValLimit = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.PassiveSubtask.WarningLevel.PositiveWholeValLimit",strLanguage);
	String limitDecimalNumbers = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.PassiveSubtask.WarningLevel.LimitDecimalNumbers",strLanguage);
...
/* Start : IR-751059 */
/*******************************************************************************/
/* function checkParentChildDependancy()                                                 */
/* Checks if there is any depedancy between Parent-Child Tasks   */
/* Parent-Child(i.e. A<->AB and A<->AB<->ABC or A<->ABC) */
/* if YES then returns false                                              */
/*******************************************************************************/
function checkParentChildDependancy(strEnteredDependancy,cellIdValue,cellRowId){
	var parentRowId = "";
	var taskName;
	var errorUrl = "";
	var isPCDependency = false;
	
	if(parseInt(strEnteredDependancy) == 0) {
		if(window && window.dataGridEnabled) errorUrl = "../";
		errorUrl = errorUrl + "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.Common.Dependency.InvalidDependencyOnProject&msgValue=";
		var errorMessage = emxUICore.getData(errorUrl);
		alert(errorMessage);
		return false;
	}
	
	if(window && window.dataGridEnabled){
		var matchedNode = emxEditableTable.getNodeFromAttribute("ID", strEnteredDependancy);
	    parentRowId = emxEditableTable.getAttribute(matchedNode ,"rowId");
		if(cellRowId.startsWith(parentRowId) || parentRowId.startsWith(cellRowId)){
			isPCDependency = true;
			taskName = "name";
			errorUrl = "../";
		}
	} else if(window && (window.name == "PMCWBSFlatView")) {
		var headerTable = document.getElementById("headTable");
		var headerRows = headerTable.rows;
		var cellInd = headerRows[2].cells["WBS"].cellIndex;
		var levelCellIndex = cellInd/2;
		
		var table = document.getElementById("bodyTable");
		var tableRows = table.rows;
		
		var enteredDepRow = tableRows[parseInt(strEnteredDependancy)];
		if(enteredDepRow){
			var enteredLevelCell = enteredDepRow.cells[levelCellIndex];
			if(enteredLevelCell){
				var enteredLevelValue = enteredLevelCell.innerHTML;
			}
		}
		
		var currentCellRow = tableRows[parseInt(cellIdValue)];
		if(currentCellRow){
			var currentLevelCell = currentCellRow.cells[levelCellIndex];
			if(currentLevelCell){
				var currentLevelValue = currentLevelCell.innerHTML;
			}
		}
		if(enteredLevelValue && enteredLevelValue!="" && currentLevelValue && currentLevelValue!=""){
			if(currentLevelValue.startsWith(enteredLevelValue) || enteredLevelValue.startsWith(currentLevelValue)){
				isPCDependency = true;
				taskName = "ID";
			}
		}
		...
		for (i = 0, tableRows; tableRows = table.rows[i]; i++) {
			var currentCell = tableRows.cells[IdCellIndex];
			if(currentCell){
				var currentIdValue = currentCell.innerHTML;
				if(currentIdValue === strEnteredDependancy){
					currentRowId = currentCell.getAttribute("rmbrow");
					if(cellRowId.startsWith(currentRowId) || currentRowId.startsWith(cellRowId)){
						isPCDependency = true;
						taskName = "ID";
					}
				}
	...
	if(isPCDependency){
		taskName = emxEditableTable.getCellValueByRowId(cellRowId,taskName).value.current.display; 
		errorUrl = errorUrl + "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessageWithPlaceHolder&errorKey=emxComponents.Task.CircularDependancyDetected&msgValue="+taskName;
		var errorMessage = emxUICore.getData(errorUrl);
		alert(errorMessage);
		return false;
	}
...
function checkTransitiveDependancy(strEnteredDependancy,currentCellIdValue,editatedRowId){
	...
	var expectedRow = "";
		if(window && (window.name == "PMCWBSFlatView")){
			expectedRow = emxEditableTable.getCellValueByRowId(expectedRowId,"Predecessor");
		}
		else{
			expectedRow = emxEditableTable.getCellValueByRowId(expectedRowId,"Dependency");
		}
		if(null!=expectedRow){
			...
			if(""!=expectedValue){
				 ...
				 for(var i=0; i < allVals.length ; i++) {
				  ...
				  if(dep==currentCellIdValue){
					var taskName = "";
					var errorUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessageWithPlaceHolder&errorKey=emxComponents.Task.CircularDependancyDetected&msgValue=";
					if(window && window.dataGridEnabled) {
						taskName = emxEditableTable.getCellValueByRowId(editatedRowId,"name").value.current.display;
						errorUrl = "../"+errorUrl;
					}
					else if(window && (window.name == "PMCProjectTemplateWBS")){
						taskName = emxEditableTable.getCellValueByRowId(editatedRowId,"Task Name").value.current.display;
					}
					else{
						taskName = emxEditableTable.getCellValueByRowId(editatedRowId,"ID").value.current.display;
					}
					errorUrl +=  taskName;
					var errorMessage = emxUICore.getData(errorUrl);
					alert(errorMessage);
...
/* --------------------------- Validate circular dependency Start ---------------------------------- */
  	...
  	/*---------------------------- Validate self dependency--------------------------- */
		...
		var errorUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessageWithPlaceHolder&errorKey=emxComponents.Task.CircularDependancyDetected&msgValue=";
		if(window && window.dataGridEnabled) {
			taskName = emxEditableTable.getCellValueByRowId(uid,"name").value.current.display;
			errorUrl = "../"+errorUrl;
		}
		errorUrl +=  taskName;
		var errorMessage = emxUICore.getData(errorUrl);
		...
		if(window && (window.name == "PMCWBSFlatView")){
			emxEditableTable.setCellValueByRowId(uid,"Predecessor",cCellValue,cCellValue);
		}
		else{
			emxEditableTable.setCellValueByRowId(uid,"Dependency",cCellValue,cCellValue);  // Added to fix issue related to circular depedancies ( similar to IR-662730 )
		}
		return false;
		}else{
	/*---------------------------- Validate Transitive dependency--------------------- */
		...
		if(!noCirculatDependency){
			...
			if(window && (window.name == "PMCWBSFlatView")){
				...
				emxEditableTable.setCellValueByRowId(uid,"Dependency",cCellValue,cCellValue); 
			}
			return false;
				}
			
	/* Start : IR-751059 */
	/*---------------------------- Validate Parent-Child dependency--------------------- */
		var noParentChildDependency = checkParentChildDependancy(dependencyNumber,currentCellIdValue,uid);
		if(!noParentChildDependency){
			/*---------------------------- New--------------------- */
			if(cCellValue == null){
				cCellValue = "";
			}
...
/* ---------------------------- Validate circular dependency End ------------------------------------------ */
    ...
    {
        //var regexp = /^(((\w+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s[DdHhM])?)(\,((\w+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s[DdHhM])?))*)?$/;
		var STR_DEC_SYM = "<%=FrameworkProperties.getProperty( "emxFramework.DecimalSymbol")%>";
		if(STR_DEC_SYM == ",")
		{
			var regexp = /^((([a-zA-Z0-9_-]+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?)(\,((\w+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?))*)?$/;
		}
		else
		{
        var regexp = /^((([a-zA-Z0-9_-]+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?)(\,((\w+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?))*)?$/;
...
/* ---------------------------- Validate circular dependency End ------------------------------------------ */
    ...
    if(!isEmpty(dependencyVal))
    {
		...
		if(STR_DEC_SYM == ",")
		{
			var regexp = /^((\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?)(\,(\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?))*)?$/;
		}
		else
		{
        	var regexp = /^((\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?)(\,(\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhM]|[Mm][Oo]|[Mm][Oo][Nn]))?))*)?$/;
...
function isBadNameCharsSB(){
    ...
    for (var i=0; i < ARR_NAME_BAD_CHARS.length; i++){
		if (ARR_NAME_BAD_CHARS[i] != '' && fieldValue.indexOf(ARR_NAME_BAD_CHARS[i]) > -1){
...
function validateBadChars(){
    ...
    var url = "../programcentral/emxProgramCentralUtil.jsp?mode=getBadCharsNotName";
    //To fix IR-980816-3DEXPERIENCER2022x.
    if(window.dataGridEnabled) {
    	url = "../"+url;
    }
...
/*****************************************************************************************************/
/* function isValidWarningThreshold() - checks whether the value of WarningThreshold is Valid numeric*/
/*                                                                                                   */
/*****************************************************************************************************/

function isValidWarningThreshold()
{
	debugger;
    var warningThresholdVal = arguments[0];
    if (warningThresholdVal == "" || !isValidNumeric(warningThresholdVal)) 
	{
		alert("<%=numeric%>");
		return false;
	}
	else if(null != warningThresholdVal){
			
			if (!(warningThresholdVal % 1)) {
		        if(warningThresholdVal < 0){
		            alert("<%=positiveWholeVal%>");
		            return false;
		        }else if(warningThresholdVal > 1000){
		            alert("<%=positiveWholeValLimit%>");
		            return false;
		        }
			} else {
		      alert("<%=limitDecimalNumbers%>");
		      return false;
			}
    	}
    	return true;
