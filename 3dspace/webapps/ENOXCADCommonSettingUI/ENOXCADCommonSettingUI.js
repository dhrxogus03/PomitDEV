...
define('DS/ENOXCADCommonSettingUI/utils/CustomTableRowUtil', [
    ...
    var customTableRowUtil = {
        ...
        createAttributesComboBoxCell: function() {
            const attributesComboBoxCell = UWA.createElement('td', {
                width: '85%',
                align: 'left',
                styles: {
                    'padding-top': '5px',
            ...
            return attributesComboBoxCell;
        },

        createAddAttributeButtonCell: function() {
            const addAttributeButtonCell = UWA.createElement('td', {
                width: '15%',
                align: 'left'
            });

            return addAttributeButtonCell;
        },
        ...
        createAttributesComboBox: function(rangeLables, rangeValues) {
            const elementsList = [];

            for (let i = 0; i < rangeValues.length; i++) {
                elementsList.push({
                    labelItem: rangeLables[i],
                    valueItem: rangeValues[i]
                });
            }

            const attributesComboBox = new WUXComboBox({
                elementsList: elementsList,
                enableSearchFlag: false,
                actionOnClickFlag: false
            });

            attributesComboBox.elements.container.style.width = '100%'; // in px or %
            // attributesComboBox.elements.container.style.padding = '5px 5px 5px 0px';
            ...
            return new Promise((resolve, reject) => {
                ...
                WAFData.authenticatedRequest(url, {
                    ...
                    onFailure: function(error, response) {
                        if (response)
                            console.log('Error from server side: ', response.errorMessage);
...
define('DS/ENOXCADCommonSettingUI/Views/XCADCustomTableRowView', [
    ...
    var customTableRowView = View.extend({
        ...
        render: function() {
            ...
            const attributesComboBoxCell = CustomTableRowUtil.createAttributesComboBoxCell();
            attributesComboBoxCell.inject(addSelectionChipsRow);

            const allAttributesComboBox = CustomTableRowUtil.createAttributesComboBox(
                rangeLabels,
                rangeValues
            );

            allAttributesComboBox.inject(attributesComboBoxCell);

            const addAttributeButtonCell = CustomTableRowUtil.createAddAttributeButtonCell();
            addAttributeButtonCell.inject(addSelectionChipsRow);

            const addAttributeButton = CustomTableRowUtil.createAddAttributeButton();
            addAttributeButton.inject(addAttributeButtonCell);

            // Add type button on click event handler
            addAttributeButton.addEventListener('click', function() {
                const addedChipsAsLabels = selectionChips.getAllChipsAsLabels();

                if (!addedChipsAsLabels.includes(allAttributesComboBox.label)) {
                    selectionChips.addChip({
                        label: allAttributesComboBox.label,
                        value: allAttributesComboBox.value
                    });
                } else {
                    AlertMessage.add({
                        className: 'error',
