...
function (UWA, Abstract, CXPCameraViewerCSS, DomUtils, PointerEvents, RenderUtils, WebGLV6Viewer, Viewpoint) {
    ...
    function _RefreshViewerFromCamera() {
          ...
          */
          {
            ...
            _applyCamera: function() {
                if (!this._cameraViewer) {
                    if (this.camera) {
                        _BuildViewer.apply(this);
                    } else {
                        return;
                    }
                }

                _RefreshContent.apply(this);
                _RefreshViewerFromCamera.apply(this);

                let visuMgr = this.camera._experienceBase.getManager('CAT3DXVisuManager');
                if (this.liveUpdate && !this.disabled) {
                    if (!this._updateCallback) {
                        this._updateCallback = _RefreshViewerFromCamera.bind(this);
