...
function getRouteIdFromContentIdAndLaunch(){
	if(parent.window.getContextObjDetails){
		var routeId = "";
		var contentId = parent.window.getContextObjDetailsFromPortal().physId;
		if(typeof contentId == 'undefined' || contentId === ""){
			contentId = parent.window.getContextObjDetails().physId;
			routeId = emxUICore.getData("emxLifecycleResolveUserGroup.jsp?objectId="+contentId);
		} else {
			try {
				routeId = emxUICore.getData("emxLifecycleResolveUserGroup.jsp?objectId="+contentId);
			} catch(e){
