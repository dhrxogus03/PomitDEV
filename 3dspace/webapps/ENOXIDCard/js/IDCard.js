        ...
        _createTooltipWebUX : function(){
            ...
            if(that.model.get('attributes')){
                that.model.get('attributes').forEach((attribute, i) => {
                    var containerCollapsed, containerExpanded,containerIconCollapsed,containerIconExpanded,containerImageCollapsed,containerImageExpanded;
                    var tooltipIcon, tooltipImage;
                    if(!attribute.container){
                        var tooltipValue=new WUXTooltipModel({
                            shortHelp: attribute.tooltipValue || attribute.value
                        });
                        ...
                        if(attribute.displayWhenMinified){
                            ...
                            setWebUXTooltipContent(containerCollapsed,tooltipValue);
                            containerExpanded = that.container.getElements("#"+attribute.id)[1];
                            setWebUXTooltipContent(containerExpanded,tooltipValue);

                            // tooltip on icon
                            containerIconCollapsed= that.container.getElements("#"+attribute.id+"-icon")[0];
                            setWebUXTooltipContent(containerIconCollapsed,tooltipIcon);
                            containerIconExpanded = that.container.getElements("#"+attribute.id+"-icon")[1];
                            setWebUXTooltipContent(containerIconExpanded,tooltipIcon);

                            // tooltip on image
                            containerImageCollapsed= that.container.getElements("#"+attribute.id+"-image")[0];
                            setWebUXTooltipContent(containerImageCollapsed,tooltipImage);
                            containerImageExpanded = that.container.getElements("#"+attribute.id+"-image")[1];
                            setWebUXTooltipContent(containerImageExpanded,tooltipImage);
                            ...
                            setWebUXTooltipContent(containerExpanded,tooltipValue);
                            // tooltip on icon
                            containerIconExpanded = that.container.getElements("#"+attribute.id+"-icon")[0];
                            setWebUXTooltipContent(containerIconExpanded,tooltipIcon);
                            // tooltip on image
                            containerImageExpanded = that.container.getElements("#"+attribute.id+"-image")[0];
                            setWebUXTooltipContent(containerImageExpanded,tooltipImage);
                        }
                    }
                });
          ...
          if(maturityContainer) {
            ...
            require(requiredMods,function(MaturityCompactView){
                ...
                if(that.maturityCmd.container){
                    that.maturityCmd.container.addEvent("resize",function(){
                        that.resize();
                    });
                }
            });
