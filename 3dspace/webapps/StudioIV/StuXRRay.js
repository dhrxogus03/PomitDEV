	...
	function (STU, Behavior, Task, Tools, EP, DSMath, EventServices, XRManager) {
		...
		*/
		XRRay.prototype.onActivate = function (oExceptions) {
			...
			this._XRMgr = STU.XRManager.getInstance();
		...
		*/
		...
		XRRay.prototype.onExecute = function (iContext) {
			...
			if (this.display === true) {
				var worldCtrlTransform = this.getActor().getTransform();
		...
		XRRay.prototype.pick = function () {
			...
			var firstIntersection = STU.RenderManager.getInstance().pickFromRay(ray, { pickAllElements: true });
		...
		*/
		...
		XRRay.prototype._onDevicePressEvent = function (iDeviceEvent) {
			...
			if (this.display !== true || iDeviceEvent.buttonName !== XRManager.EDeviceInputName[this.clickInputID] + "_" + controllerID) {
				return;
			}
