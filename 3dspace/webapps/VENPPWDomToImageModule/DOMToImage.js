define('DS/VENPPWDomToImageModule/DOMToImage',
    [
        'UWA/Core'
    ],
    function (UWA) {
        ...
        var DOMToImage = function DOMToImage(
            ...
            function cloneNode(node, options, parentComputedStyles, ownerWindow) {
                ...
                function processClone(clone, original) {
                    ...
                    function cloneStyle() {
                        copyStyle(original, clone.style);
                        ...
                        function copyStyle(sourceElement, targetElement) {
                            ...
                            if (sourceComputedStyles.cssText) {
                                targetElement.cssText = sourceComputedStyles.cssText;
                                copyFont(sourceComputedStyles, targetElement); // here we re-assign the font props.
                            }
                            else {copyProperties(sourceComputedStyles, targetElement);}
                            function copyProperties(sourceComputedStyles, targetElement) {
                                util.asArray(sourceComputedStyles).forEach(function (name) {
                                    targetElement.setProperty(
                                        name,
                                        sourceComputedStyles.getPropertyValue(name),
                                        sourceComputedStyles.getPropertyPriority(name)
                                    );
                                });
                            }
                        }
            ...
            return {
                captureSVG: function captureSVG(node, opts) {
                    var svg;
                    if (UWA.is(node)) {
                        svg = toSvg(node, opts);
                    }
                ...
                capturePNG: function capturePNG(node, opts) {
                    var png;
                    if (UWA.is(node)) {
                        png = toPng(node, opts);
                    }
