...
export const useCallHistoryStore = defineStore('CallHistory', () => {
  ...
  const startCall = function (data) {
    if (data.logins && data.logins.length > 0) {
      if (!data.logins.includes(currentUserLogin.value)) data.logins.push(currentUserLogin.value);
  ...
  const deleteCall = function (deleteId) {
    if (deleteId instanceof Array) {   // For deleting merged calls
      deleteId.forEach(id => {
        callsMap.delete(id);
        callIdList = callIdList.filter(callId => callId !== id);
      });
    }
    else {
      callsMap.delete(deleteId);
      callIdList = callIdList.filter((callId) => callId != deleteId)
    }
  }
  ...
  const processCallHistoryData = function (callHistory) {
    ...
    if (callHistory && callHistory.data && callHistory.data.calls) {
      ...
      if (callHistory.data.calls instanceof Array) {
        ...
        calls = Object.values(callHistory.data.calls);
        if (calls && calls.length < limit.value) {
          moreCallsToFetch.value = false;
        }
        else moreCallsToFetch.value = true;
  ...
  return {
    filters, selectedContact, isLoading, moreCallsToFetch, waitForResponse, isRTCStoreLoaded, filteredCalls, enableTelephony, enableProfile, currentUserLogin, currentUserId, showAlert, remainingUsersToAdd,
    updateOngoingCall, processCallHistoryData, emptyHistory, deleteCall, getLastCallDate, fetchCallHistory, fetchNextPage, startCall, updateUwpVars, updateFilters, searchContacts
  }
})