...
define('DS/ENOUserGroupMgmt/Services/UserGroupServices',
    ...
    let UGServices,_switchuserAccess,_revokeAccessFromUserGroup, _fetchAllGroups,_createUserGroups,_duplicateUserGroup, _updateState, _deleteUserGroups, _fetchUGMembers,_addMembersToGroup,_fetchUserGroupById,_removeMembersFromUserGroup, _fetchUserGroupAccessRights, _filterUserGroups, _addAccessRightssToGroup, _makeWSCall;
    ...
    _filterUserGroups = function(filterType){
            return new Promise(function(resolve, reject) {
   			 	let postURL= "../../resources/bps/application/filter?filterType="+filterType;
                let options = {};
                options.method = 'GET';
                options.headers = {
                        'Content-Type' : 'application/json',
    ...
    UGServices={
            ...
            filterUserGroups : (filterType) => {return _filterUserGroups(filterType);},
...
function(UserGroupServices, Promise) {
 ...
 /*   let commonFailureCallback = function(data){  
    ...
    UserGroupServicesController = {
            ...
            filterUserGroups : function(filterType) {
            	return new Promise(function(resolve, reject) {
                    UserGroupServices.filterUserGroups(filterType).then(function(response) {
                        resolve(response);
                    },function(response) {
                        reject(response);
                    });
				...
				menu:[
					{
					 ...
					 {
						type:'CheckItem',
						title: NLS.FilterforMemberAccess,    // UG105941  : NLS
						  ...
						  action: {
							  ...
							  argument: {
								  "type":"owner",
								  "filter":"access:member"
							  }
						  },
				      },
				      {
						type:'CheckItem',
						title: NLS.FilterforManagerAccess,    // UG105941  : NLS
						  ...
						  action: {
							  ...
							  argument: {
								  "type":"owner",
								  "filter":"access:manager"
							  }
						  },
				      },
				      {
						type:'CheckItem',
						title: NLS.FilterforOwnerAccess,    // UG105941  : NLS
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupSummaryView',
	...
	let getFilterPreferences = function(){
    		...
    		return ['owned','all',"access:owner","access:manager","access:member"];
	...
	let addorRemoveUserGroupEventListeners = function(){
		...
		ugSyncEvts.ugMgmtMediator.subscribe('usergroup-summary-append-rows', function (data) {
			let grpid = data.pid || null;
			
				UserGroupServicesController.fetchUserGroupById(grpid,data.uri).then(function(resp) {
...
define('DS/ENOUserGroupMgmt/Actions/Toolbar/UsersGroupSummaryToolbarActions',
		[	'DS/ENOUserGroupMgmt/Views/Grid/UsersGroupSummaryDataGridView',
			...
			'DS/ENOUserGroupMgmt/Utilities/UserGroupSpinner',
			'i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt'
			], function(UGSummaryDataGridView, UGSummaryView, UserGroupServicesController,UserGroupSpinner, NLS) {
    ...
    var applyfilterView = function(view,option){
        ...
        //if(view.type == "owner"){  //???
        	...
        	else if(view.filter.indexOf("access">-1)) {
				filterType = view.filter.substring(("access".length)+1);
			}
			UserGroupSpinner.doWait(document.body);
			UserGroupServicesController.filterUserGroups(filterType)
				.then((success) => {
					//console.log(success);
					UGSummaryView.filterUserGroupSuccess(success);
					UGSummaryDataGridView.selectFilterPreferences(filterType);
				})
				.catch((error) => {  
					//do nothing else
