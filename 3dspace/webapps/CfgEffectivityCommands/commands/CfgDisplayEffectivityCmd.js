define('DS/CfgEffectivityCommands/commands/CfgDisplayEffectivityCmd', [
    ...
    var CfgDisplayEffectivityCmd = CfgEffCmd.extend({
        ...
        getOccurrenceData: async function () {
            ...
            let instancePathIds = [];
            let totalOccurrenceIds = [];
            if (typeof data.padNodes[0].getOccurencePathWithType == 'function') {
                totalOccurrenceIds = data.padNodes[0].getOccurencePathWithType();
            } else {
                //some application doesn't support getOccurencePathWithType like Manufacturing Item Management for which Occurrence expander should be hidden.
                return data;
            }
            let occurrenceLength = totalOccurrenceIds.length - 1;
            for (let counter = 0; counter < occurrenceLength; counter++) {
                if (totalOccurrenceIds[counter].type == 'VPMReference') referencePathIds.push(totalOccurrenceIds[counter].resourceid);
                else if (totalOccurrenceIds[counter].type == 'VPMInstance' || totalOccurrenceIds[counter].type == '') instancePathIds.push(totalOccurrenceIds[counter].resourceid);
                //added '' check in OR condition, since type value for VPMInstance is '' for non-indexed data.
            }
        ...
        getOccurrenceView: async function (iOptions) {
            ...
            if (effViewData.instanceID != undefined && data.selectedNodes.length == 1 && effViewData.instanceID.length == 1 && effViewData.instanceID[0] == instanceId) {
                if (occurrenceRequired == true) {
                    ...
                    let occurrenceOptions = { effViewData: effViewData, occurrencesIds: data.selectedNodes[0].instancePathIds, referencePathIds: data.selectedNodes[0].referencePathIds, occurrenceRequired: occurrenceRequired };
                ...
                let probesOptions = { AppId: magicAppId, AppName: magicAppName, CommandName: CfgTracker.Labels['DISPLAY_EFFECTIVITY'], CommandOperation: 'Display_Effectivity_Dialog_Open', ConfigMode: CfgTracker.ConfigMode['CONTEXTUAL'], ApplicationMode: CfgTracker.ApplicationMode['DASHBOARD'] };
        ...
        _processOccurrenceDialogLaunch: async function (data) {
            ...
            await that.cfgOccurrenceEffViewFactory.loadEffectivities(instanceId, tenantId).then(
                async function (effViewData) {
                    let iOptions = { effViewData: effViewData, data: data, occurrenceRequired: occurrenceRequired, instanceId: instanceId };
                ...
                if (data.selectedNodes[0].isRoot == true) {
                    ...
                    CfgUtility.populate3DSpaceURL().then(function () {
                        CfgUtility.populateSecurityContext().then(function () {
                            ...
                            var cfgDispEffIconDiv = new UWA.createElement('div', {
                            ...
                            if (data.padNodes != undefined && data.padNodes.length == 1 && typeof data.padNodes[0].getIcons === 'function' && data.padNodes[0].getIcons().length == 1 && data.padNodes[0].getIcons()[0].iconPath != undefined) {
                                ...
                                new UWA.createElement('img', { src: iconImagePath }).inject(cfgDispEffIconDiv);
                            }
