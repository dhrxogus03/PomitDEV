define('DS/MePreferencesUIBuilder/MePreferencesPageModelDataProvider',
    ['DS/WAFData/WAFData',
    ...
    function (WAFData, NotificationsManagerUXMessages, WUXNotificationsManagerViewOnScreen, MePreferencesClientCacheMgr) {
        ...
        mePreferencesPageModelDataProvider.prototype.getPageModel = async function (requestedURL, appID, currentLanguage) {
            if (!appID.contains(",")) { //cache not applicable for multiple appIDs
                ...
                if (!data) {
                    ...
                    data = await getResourcesFromServer(requestedURL, clientEtag);
                    if (data) {
        ...
        async function getResourcesFromServer(url, clientEtag) {
            ...
            return await callRestAPI.call(this, REQUEST_BODY, url, clientEtag, 'GET').then((responseObject) => {
