									...
									if (UWA.is(baseUrl, 'string')) {
										...
										csrfOptions.onComplete = function (response, headers) {
											...
											SearchUtils.getSecurityContextPromise().then(function () {
												...
												fetchOptions.onFailure = function (error, response, headers) {
													...
													if(joPartDeleteResponse.internalError.contains("Check trigger blocked event")){
														partActionHelper.displayAlert({"message":(joPartDeleteResponse.error == undefined ? NLS.PartDeleteError : joPartDeleteResponse.error),"className":"error"});
													}
													else {
										...
										csrfOptions.onFailure = function (error, response, headers) {
											...
											if(joCSRFResponse.internalError.contains("Check trigger blocked event")){
												partActionHelper.displayAlert({"message":(joCSRFResponse.error == undefined ? NLS.PartSpecDeleteError : joCSRFResponse.error),"className":"error"});
											}
											else {
