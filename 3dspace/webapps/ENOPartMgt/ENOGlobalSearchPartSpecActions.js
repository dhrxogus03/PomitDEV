								...
								'onComplete': function (baseUrl) {
									if (UWA.is(baseUrl, 'string')) {
										...
										csrfOptions.onComplete = function (response, headers) {
											...
											SearchUtils.getSecurityContextPromise().then(function () {
												...
												var joPartSpecDeleteResponse = JSON.parse(response);
													if(joPartSpecDeleteResponse.internalError.contains("Check trigger blocked event")){
														partSpecActionHelper.displayAlert({"message":(joPartSpecDeleteResponse.error == undefined ? NLS.PartSpecDeleteError : joPartSpecDeleteResponse.error),"className":"error"});
													}
													else {
										...
										csrfOptions.onFailure = function (error, response, headers) {
											...
											if(joCSRFResponse.internalError.contains("Check trigger blocked event")){
												partSpecActionHelper.displayAlert({"message":(joCSRFResponse.error == undefined ? NLS.PartSpecDeleteError : joCSRFResponse.error),"className":"error"});
											}
											else {
