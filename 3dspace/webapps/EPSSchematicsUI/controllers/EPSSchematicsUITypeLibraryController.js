    ...
    class UITypeLibraryController {
                    ...
                    if (areSettable) {
                        selectedDataPorts.forEach((dataPort, index) => {
                            if (index === 0) {
                                ...
                                allowedValueTypes = allowedValueTypes.filter(valueType => dataPort.getModel().isValueTypeSettable(valueType));
                            }
        ...
        updateOccurenceCount() {
            ...
            if (typeLibraryPanel !== undefined && typeLibraryPanel.isOpen()) {
                ...
                const typeNodes = [].concat(this._customTypesNode.getChildren() || [], this._appTypesNode.getChildren() || []);
        ...
        _registerLocalCustomType(typeName) {
            ...
            if (selectedDataPorts.length > 0) {
                const areSettable = selectedDataPorts.every(dataPort => dataPort.getModel().isValueTypeSettable());
                if (areSettable) {
                    disabled = !selectedDataPorts.every(dataPort => (dataPort.getModel().typeCategory & ModelEnums.FTypeCategory.fObject) === ModelEnums.FTypeCategory.fObject);
                }
            }
