...
define("DS/EPSSchematicsUI/datagrids/EPSSchematicsUIDGVDataItemTools", ["require", "exports", "DS/EPSSchematicsModelWeb/EPSSchematicsModelEnums", "DS/EPSSchematicsModelWeb/EPSSchematicsTypeLibrary", "DS/EPSSchematicsUI/typings/WebUX/EPSWUXTreeNodeModel", "DS/EPSSchematicsUI/datagrids/EPSSchematicsUIDGVTools", "DS/EPSSchematicsUI/tools/EPSSchematicsUITools", "i18n!DS/EPSSchematicsUI/assets/nls/EPSSchematicsUINLS", "DS/EPSSchematicsModelWeb/EPSSchematicsControlPort", "DS/EPSSchematicsModelWeb/EPSSchematicsEventPort", "DS/EPSSchematicsModelWeb/EPSSchematicsDataPort", "DS/EPSSchematicsModelWeb/EPSSchematicsSetting"], function (require, exports, ModelEnums, TypeLibrary, WUXTreeNodeModel, UIDGVTools, UITools, UINLS, ControlPort, EventPort, DataPort, Setting) {
    ...
    class UIDGVDataItemTools {
         ...
         * FIXME: This method is not specific to data item but to model item => move it into DGVTools!
         * @public
         * @static
         * @param {IWUXCellInfos} cellInfos - The cell infos.
         * @param {TModelItemDataIndex} dataIndex - The column data index.
            ...
            if (nodeModel && dataItem) {
                ...
                if (isDataPort || isSetting || isEventPort) {
                    ...
                    if (isSettable) {
                        ...
                        const hasCreateTypeButton = (dataItem.typeCategory & ModelEnums.FTypeCategory.fObject) === ModelEnums.FTypeCategory.fObject;
                        ...
                        const isLocalType = TypeLibrary.hasLocalCustomType(graphContext, valueType, typeCategory);
                        result = {
                            editor: editor,
        ...
        static _setValueTypeCellValueForDataItem(nodeModel, newValueType, dataItem) {
            if (nodeModel && dataItem && dataItem.isValueTypeSettable()) {
                ...
                if (result) {
                    ...
                    if (globalThis.EPSDGVPROTO === true) {
         ...
         */
        static getDataItemDefaultValueCellEditableState(cellInfos, dataIndex, readOnly) {
            let result = false;
            const nodeModel = cellInfos?.nodeModel;
            if (nodeModel && !readOnly) {
                const isSection = nodeModel.getAttributeValue('isSection');
                ...
                if (!isSection && editor) {
                    const dataItem = nodeModel.getAttributeValue(dataIndex);
                    if (dataItem) {
                        result = this._isDataItemDefaultValueEditable(dataItem, editor);
                    }
                    else {
                        result = this._isObjectItemDefaultValueEditable(nodeModel, dataIndex);
                    }
                }
            }
            return result;
         ...
         */
        static _isDataItemDefaultValueEditable(dataItem, editor) {
            let result = false;
            if (dataItem && dataItem.block) {
                ...
                result = !isOutputDataPort;
                result = result && (isSetting || isLocalDataPort || !UIDGVTools.isReadOnlyRoot(editor, dataItem)); // Check only for data port is not root (CSI specific restriction)!
                ...
                result = result && !UIDGVTools.isEmptyLocalCustomType(dataItem);
            }
            return result;
        }
        /**
         ...
         */
        static _isObjectItemDefaultValueEditable(nodeModel, dataIndex) {
            let result = false;
            if (nodeModel) {
                ...
                const isRootDataItemEditable = !rootDataItem || (rootDataItem && this._isDataItemDefaultValueEditable(rootDataItem, editor));
         ...
         * @param {WUXTreeNodeModel} nodeModel - The node model.
         * @param {TDataItemDataIndex} dataIndex - The column data index.
         * @returns {DataPort|Setting} The root data item.
         */
        ...
        static getDataItemDefaultValueCellTypeRepresentation(cellInfos, dataIndex) {
            ...
            if (nodeModel) {
                const valueType = nodeModel.getAttributeValue('valueType');
                const hasDataIndex = dataIndex !== undefined; // Only useful if using ObjectEditor tweaker!
                const graphContext = nodeModel.getAttributeValue('graphContext');
                const isGenericObjectType = UIDGVTools.isGenericObjectType(valueType);
                const isGenericArrayType = UIDGVTools.isGenericArrayType(valueType);
            ...
            if (nodeModel) {
                ...
                if (!isSection) {
                    ...
                    if (dataItem) {
                        ...
                        result = isDefaultValueValid /* && !UIDGVTools.isEmptyLocalCustomType(dataItem)*/ ? defaultValue : undefined;
                        ...
                        if (isDataPort) {
                            ...
                            if (traceController.getPlayingState()) {
                                ...
                                const parentDataPortModel = isSubDataPort ? dataItem.dataPort : dataItem;
                                const events = traceController.getEventByDataPortPath(parentDataPortModel.toPath());
                                if (events.length > 0) {
                                    const fromDebug = nodeModel.getAttributeValue('fromDebug');
                                    const event = events[events.length - 1];
                                    result = fromDebug ? defaultValue : (isSubDataPort ? event.getValue()[dataItem.getName()] : event.getValue());
                                }
                            }
                        }
                    }
                        ...
                        // FIXME: Check no regression displaying object with bigint!!!
                        result = defaultValue;
                    }
                }
            }
                ...
                if (!isSection) {
                    ...
                    if (dataItem) {
                        ...
                        if (isValueSettable) {
                            ...
                            const result = isDataPort ? (isTestValue ? true : dataItem.setDefaultValue(newValue)) : dataItem.setValue(newValue);
                            if (result) {
                                const value = result ? newValue : previousValue;
                        ...
                        if (newValue !== previousValue) {
                            this._updateValueAndPropagateToParent(nodeModel, dataIndex, newValue, valueIndex);
                        }
                    }
                }
            }
        ...
        static _getActionsCellSemanticsForDataItem(cellInfos, dataItem, valueIndex) {
            ...
            if (nodeModel) {
                ...
                const isRootDataItemEditable = !isDataPort || (isDataPort && this._isDataItemDefaultValueEditable(dataItem, editor));
                if (isRootDataItemEditable || isTestDataPort) {
                    ...
                    const fullDisplay = globalThis.EPSDGVPROTO === true || isTestValue;
                    result = {
                        count: fullDisplay ? 4 : 2,
                        ...(!isTestValue && {
        ...
        static setModelItemSectionActionsCellValue(cellInfos, dataIndex, value) {
            if (value) {
                switch (value) {
                    case 'eAddAction': // FIXME: Migrate to an enum!
        ...
        static setDataItemActionsCellValue(cellInfos, dataIndex, valueIndex, value) {
            if (value) {
                switch (value) { // FIXME: Migrate to an enum!
                    case 'eAddAction':
        ...
        static _onAddAction(cellInfos, dataIndex, valueIndex) {
            ...
            if (nodeModel) {
                ...
                if (!isSection) {
                    ...
                    const isGenericObjectType = UIDGVTools.isGenericObjectType(valueType);
                    let propertyName, propertyValueType, propertyDefaultValue, canExpand;
        ...
        static _updateValueAndPropagateToParent(nodeModel, dataIndex, value, valueIndex) {
            if (nodeModel !== undefined) {
                /*if (typeof value === 'object') {
                    ...
                    nodeModel.updateOptions(options);
                    //(nodeModel as any).notifyModelUpdate({ actions: {}}, { actions: undefined});
                    // FIXME: nodeModel.notifyModelUpdate();
                    // FIXME:  + update ref object et array
                ...
                if (isNotRootDataIndex || isNotRootObjectEditor) {
                    ...
                    if (parentNodeModel !== undefined) {
                        let parentValue = parentNodeModel.getAttributeValue(valueIndex);
                        if (parentValue === undefined) {
                            const parentValueType = parentNodeModel.getAttributeValue('valueType');
                            const graphContext = parentNodeModel.getAttributeValue('graphContext');
                        ...
                        this._updateValueAndPropagateToParent(parentNodeModel, dataIndex, clonedParentValue, valueIndex);
                    }
                }
                else if (!isRoot || dataItem !== undefined) {
                    const cellInfos = { nodeModel: nodeModel };
                    this.setDataItemDefaultValueCellValue(cellInfos, dataIndex, value, valueIndex);
                }
            }
        }
        /**
        ...
        static canExpand(graphContext, valueType, defaultValue) {
            ...
            result = result || UIDGVTools.isClassType(graphContext, valueType) && !this._isEmptyLocalCustomType(graphContext, valueType);
            result = result || UIDGVTools.isEventType(graphContext, valueType) && !this._isEmptyLocalCustomType(graphContext, valueType);
            return result;
        }
        /**
         * Checks if the value type is an empty local custom type.
         * FIXME: Checks if we can remove the same public API isEmptyLocalCustomType from UIDGVTools!
         * @public
         * @static
         * @param {GraphBlock} graphContext - The graph context.
         * @param {string} valueType - The value type.
        ...
        static _isEmptyLocalCustomType(graphContext, valueType) {
            const localCustomType = TypeLibrary.getLocalCustomType(graphContext, valueType);
        ...
        static _createChildrenNodeModel(nodeModel, editor, graphContext, valueIndex) {
            if (nodeModel && graphContext) {
                ...
                const valueType = nodeModel.getAttributeValue('valueType');
                if (UIDGVTools.isAnyObjectType(graphContext, valueType)) {
                    const value = nodeModel.getAttributeValue(valueIndex);
    ...
    UIDGVDataItemTools._regExpAlphanumeric = new RegExp(/^[a-z0-9_]+$/i);
    UIDGVDataItemTools._objectPossibleValues = ['Boolean', 'Double', 'String', 'Object', 'Array<Boolean>', 'Array<Double>', 'Array<String>', 'Array<Object>'];
