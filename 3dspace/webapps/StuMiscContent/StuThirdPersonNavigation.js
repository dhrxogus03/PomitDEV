define('DS/StuMiscContent/StuThirdPersonNavigation', [
	...
	function (STU, EP, Behavior, EventServices, DSMath, StuEnvServices, Actor3D) {
		'use strict';
		...
		ThirdPersonNavigation.prototype.checkParameters = function () {
			...
			if (this.keepAtDistance <= this.maximumDistance) {
				this._keepAtDistanceDelta = this.maximumDistance - this.keepAtDistance;
			} else {
		...
		ThirdPersonNavigation.prototype.updateFollowingDistance = function () {
			//note: when dealing with udaptating the dsitance, it can change from: moving the actor that we are targeting at OR changing the zoom factor

			//retrieve current camera pos (so not the ideal one)
			var currentDistanceToObject = this.DistanceToPosition(this._targetTransform.vector.clone());

			var cameraToObjectDistance = currentDistanceToObject + this._distanceDiff;
			var distanceDifferential = cameraToObjectDistance - this._keepAtDistanceZoom; //we "want" this equal to 0, to have camera at its best postiion
			...
			//applying a partial reduction to close gap the distance between cameraCurrPos and the idealPos
			cameraToObjectDistance -= distanceDifferential * timeCoef;

			// [IR-871915] Re-open: Added this so that when we are going too far from desired position, cam is pulled in (ex: going too fast) (+ ensure zoom in while moving)
			if (cameraToObjectDistance > this._keepAtDistanceZoom * 1.5) //arbitrary value : if above 150% of desired zoom value
			{
				cameraToObjectDistance = this._keepAtDistanceZoom * 1.5;
			}

			// [IR-871915] - quick fix maximum distance only when moving
			if (cameraToObjectDistance > this.maximumDistance)
			{
				cameraToObjectDistance = this.maximumDistance;
			}

			var distance;
			//[IR-459937] To avoid dezoom when changing target
			if (this._resetCameraTransform) {
				distance = -distanceDifferential;
				this._resetCameraTransform = false;
			} else {
				distance = cameraToObjectDistance - currentDistanceToObject;
			}
			...
			//console.log('DIST: ' + currentDistanceToObject + '  Distance next: ' + distance + ' and ' + cameraToObjectDistance);
		};
