define('DS/StuMiscContent/StuBoatController',
	...
	function (STU, EP, Behavior, Task, EventServices, Event, DSMath, Actor3D) {
		...
		BoatController.prototype.onActivate = function (oExceptions) {
			...
			this._speedVec = new DSMath.Vector3D(0, 0, 0);
			this._posVec = new DSMath.Vector3D(0, 0, 0);
		...
		BoatController.prototype.onExecute = function (iDelta) {
			...
			if (!this.simulateWaves) {
				...
				if (waterInfo !== null && waterInfo !== undefined) {
					...
					if (waveInfo !== null && waterInfo !== undefined) {
						...
						/***** The following code is the method to use to apply a true system of forces *****/
						// var sumForce = new DSMath.Vector3D(0,0,0);//this._gravity;
						// if(!waveInfo.isOnWater)
						// {
						// 	sumForce.add(this._gravity);
						// }
						// else
						// {
						// 	var buoyancyForce = this.computeBuoyancy(deltaTime, waveInfo);
						// 	sumForce.add(buoyancyForce);
						// }
						// this._speedVec.add(sumForce.clone().multiplyScalar(deltaTime));
						// this._posVec.add(this._speedVec.multiplyScalar(deltaTime));
					}
				}
			}
			...
			if (!this._isOnSailsToState && !this._isOnFollowTrajectoryState) {
				...
				if (this._isOnSailsToState) {
					if (waveInfo !== null && waveInfo !== undefined) {
						if (waveInfo.isOnWater || this._currentSpeed != 0) {
							this.updateSailsTo(this._target.getPosition(), deltaTime);
						}
					}
					else // waves simulation activate
					{
						this.updateSailsTo(this._target.getPosition(), deltaTime);
					}
				}
			...
			//this._actor.translate(this._posVec);
		...
		BoatController.prototype.detectWater = function () {
			var upOffset = DSMath.Vector3D.multiplyScalar(this._worldUp, this._height); // previously 500
			...
			//this.drawSphere(rightRayPosition, 0.5 * 1000 * this._scale);
		...
		BoatController.prototype.computeDirectionVectors = function () {
			...
			switch (this._frontVector) {
				...
				case "-z":
					...
					this._localRight = XVector;// on suppose +X comme right axis
					this._localUp = YVector;// on suppose +Z comme up axis
					break;
				default:
					this._localFront = YVector;// on suppose +Y comme front axis
					this._localRight = XVector;// on suppose +X comme right axis
		...
		BoatController.prototype.computeSize = function () {
			...
			var highBBox = iBBox.high.applyTransformation(iLocalTransformation);
			var lowBBox = iBBox.low.applyTransformation(iLocalTransformation);

			var interPoint = highBBox.clone();
			interPoint.x = lowBBox.x;
			interPoint.y = lowBBox.y;
			var ZDimension = Math.round(lowBBox.distanceTo(interPoint) * 10) / 10;

			interPoint.x = highBBox.x;
			interPoint.z = lowBBox.z;
			var XDimension = Math.round(lowBBox.distanceTo(interPoint) * 10) / 10;

			interPoint.x = lowBBox.x;
			interPoint.y = highBBox.y;
			var YDimension = Math.round(lowBBox.distanceTo(interPoint) * 10) / 10;
			...
			center.x = (highBBox.x + lowBBox.x) * 0.5;
			center.y = (highBBox.y + lowBBox.y) * 0.5;
			center.z = (highBBox.z + lowBBox.z) * 0.5;

			//center.add(this.actor.getPosition());

			// if (this._debug) {
			//    this.drawSphere(center, 1, 1, -1);
			//   // this.drawSphere(this.actor.getPosition(), 1, 2, -1);
			// }

			var centerOffset = DSMath.Vector3D.sub(center.clone(), this.actor.getPosition().clone());

			//faire une rotation sur cet offset
			// ABA57 comment line to resolve IR-XXXX, I don't understand why we apply rotation on the offset and it cause a problem with the initPos in isOnWater
			// so I comment it, maybe need a work on centerOffset to do it properly
