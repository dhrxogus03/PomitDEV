define('DS/StuMiscContent/StuDisplayAxesBe', ['DS/StuCore/StuContext', 'DS/StuModel/StuBehavior', 'DS/StuRenderEngine/StuActor3D', 'DS/StuVirtualObjects/StuPointActor', 'DS/StuRenderEngine/StuColor'],
	function (STU, Behavior, Actor3D, PointActor, Color) {
    ...
    */
	...
	DisplayAxes.prototype.onActivate = function (oExceptions) {
        ...
        if (p !== null && p !== undefined && p instanceof Actor3D) {
	...
	DisplayAxes.prototype.createAxes = function (iDirection, iColor, iLifetime) {
		...
		if (this.scaleMode === DisplayAxes.eScaleMode.eFixedSize) {
			// this._fixedLength = this.actor.getBoundingSphere("World").getRadius();
			if (this._fixedLength <= 0 || this.actor instanceof PointActor) { // In case of empty 3DActor or PointActor
				this._fixedLength = 1000;
			}
