    ...
    */
	...
	PathActor.prototype.getValue = function (iParam, iRef) {
		...
		if (iRef !== undefined && iRef !== null) {
			...
			if (expScale !== 1) {
				// position world = position relative to the experience's
				// the experience's root could have been scaled regarding the "visu" referential (cf. StuActor3d.getTransform)
				var outputTransform = new DSMath.Transformation(new DSMath.Matrix3x3(), oCoordinates.clone());
				var refWorldTransform = DSMath.Transformation.makeScaling(expScale, new DSMath.Point(0, 0, 0));
				var invRefWorldTransform = refWorldTransform.getInverse();
				var myPosRelToRefIniRef = invRefWorldTransform.multiply(outputTransform);
				oCoordinates = myPosRelToRefIniRef.vector;
			}
		}
		return oCoordinates;
	};
		...
		if (iRef !== undefined && iRef !== null) {
			...
			if (expScale !== 1) {
				...
				oPoints.forEach(function (iPt) {
					let outputTransform = new DSMath.Transformation(new DSMath.Matrix3x3(), iPt.clone());
					let myPosRelToRefIniRef = invRefWorldTransform.clone().multiply(outputTransform);
					iPt = myPosRelToRefIniRef.vector.clone();
				}, this);
			}
		}
		return oPoints;
