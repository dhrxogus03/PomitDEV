...
define('DS/StuRenderEngine/StuRenderManager', [
	...
	function (STU, EP, Manager, Task, DSMath, Actor3D, VisuPrimitive, Color, ColorRGB) {
		...
		RenderManager.prototype._parsePickingResult = function (iIntersections, iReferential) {
			...
			var doRefTransfo = (iReferential !== null && iReferential !== undefined && iReferential instanceof Actor3D);
			if (doRefTransfo) {
		...
		RenderManager.prototype._pickFromRay = function (iRay, iReturnFirstIntersection, iPickAllElements, iReferential) {
			...
			if (iReferential !== null && iReferential !== undefined && iReferential instanceof Actor3D) {
		...
		RenderManager.prototype.getGravityVector = function (iActor, iRef) {
			...
			if (actorLocation !== null && actorLocation !== undefined) {
				...
				else if (iRef instanceof Actor3D) {
				...
				//apply the experience matrix if an environment is activated (eg: globe modify the experience global matrix)
				let currentEnv = this.getCurrentEnvironment();
				if (currentEnv !== null && currentEnv !== undefined) {
					var refWorldTransform = STU.Experience.getCurrent().getWorldTransform();
					if (refWorldTransform !== null && refWorldTransform !== undefined) {
						oGravity.applyTransformation(refWorldTransform);
					}
				}
