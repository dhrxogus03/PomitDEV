		...
		Actor3D.prototype.getTransform = function (iRef) {
			...
			if (iRef === undefined || iRef === null || iRef === "Location") {
				...
				if (myLoc !== undefined && myLoc !== null) {
					...
					let currentEnv = rm.getCurrentEnvironment();
					if (currentEnv !== null && currentEnv !== undefined) {
						//var expScale = STU.RenderManager.getInstance().getExperienceScaleFactor();
						refWorldTransform = STU.Experience.getCurrent().getWorldTransform();
						//refWorldTransform.matrix = refWorldTransform.matrix.multiplyScalar(expScale);
						iRef = null;
					}
					else {
						iRef = "World";
