...
<%
String historyReferer = (String)session.getAttribute("HISTORY_REFERER");
if ( session.getAttribute("HISTORY_REFERER") != null ){
      session.removeAttribute("HISTORY_REFERER");
}
...
function redirectToHistory(){
    var historyReferer = '<%=historyReferer%>';
    if(typeof historyReferer == 'string' && historyReferer.indexOf('historyFacet=true') >= 0){
            window.location.href = historyReferer;
    }
}
...
String onloadMethods = "initializeSlideIns();expandMenuData();setErrorHandlerWAFData();redirectToHistory();";
