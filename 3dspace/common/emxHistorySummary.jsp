...
<%
	...
	String facetLang = emxGetParameter(request, "facetLang");
	if(facetLang != null){
		requestLocale = new Locale(facetLang);
	}
    ...
    if(SpecialCaseAccesses==null || SpecialCaseAccesses.equals("")){
        SpecialCaseAccesses="change owner,change policy,change type,change name,change vault,modify form,vc connect,add interface,add ownership,vc checkin,vc checkout,vc modify,vc lock,vc unlock,vc modify connection,vc delete connection,remove interface";
    }

    if (printerFriendly != null && !"null".equals(printerFriendly) && !"".equals(printerFriendly)) {
        isPrinterFriendly = "true".equals(printerFriendly);
    }
    String sSubHeader = EnoviaResourceBundle.getProperty(context,resourceFile , requestLocale, subHeader);

    // If key is not found in resource file, try the framework file
    if ( subHeader != null && subHeader.equals(sSubHeader) )
    	sSubHeader = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, subHeader, requestLocale);
...
<%
    if(isPrinterFriendly) {
        String sHeader =EnoviaResourceBundle.getProperty(context,resourceFile , requestLocale, Header);

        // If key is not found in resource file, try the framework file
        if ( Header != null && Header.equals(sHeader) )
        	sHeader =EnoviaResourceBundle.getFrameworkStringResourceProperty(context, Header , requestLocale);
    ...
    for( int i=0; i < vectSize; i++) {  //SECTION R
        ...
        else
        {
            ...
            if(!"NoMessage".equals(sMaxRecords) && bCanDisplay ) {
                String sTooLargeMsg1 = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.LogTooLarge1" , requestLocale);
                String sTooLargeMsg2 =EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.LogTooLarge2" , requestLocale);
        ...
        if (timeArray == null || timeArray.size() == 0) {
            String noHistoryMsg = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.NoHistoryData" , requestLocale);
        ...
        for( int j=0; j < timeArray.size(); j++) {  //SECTION P
            ...
            if(ActionFilterValues.contains(sAction) || showAllHistoryItems ) {
                ...
                String formatedDate = eMatrixDateFormat.getFormattedDisplayDateTime(sTime, bDisplayTime, iDateFormat, iClientTimeOffset, requestLocale);
...
<%
	...
	for (Iterator iter = templateMapList.iterator(); iter.hasNext();) {
    	...
    	sState   = (String)historyMap.get("sState");
       	...
       	sActionI18N = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, sActionProp , requestLocale);
	...
	String tableHrdDate = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.Date" , requestLocale);
	String tableHrdUser = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.User" , requestLocale);
	String tableHrdAction = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.Action" , requestLocale);
	String tableHrdState = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.History.State" , requestLocale);
	String tableHrdDescription = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.Common.Description" , requestLocale);
%>
<!-- //XSSOK -->
...
%>
    ...
    <% 
    	if(facetLang != null){
    		%>
    		  <!-- //XSSOK -->
    	      <td nowrap><emxUtil:lzDate displaydate="true" displaytime="true" localizeparam="<%=facetLang %>" localize="i18nId" tz='<%=XSSUtil.encodeForHTMLAttribute(context, (String)session.getAttribute("timeZone"))%>' format='<%=DateFrm %>' ><%=sTime%></emxUtil:lzDate>&nbsp;
    	      </td>
    	    <%  
    	}else{
    		%>
...
<%
	...
	String sessionTZone=(String)session.getAttribute("timeZone");
    String historyReferer = emxGetParameter(request, "historyFacet");
    //System.out.println("XXXXXX timeZone:"+sessionTZone+"-historyReferer:"+historyReferer+"-");
    if(sessionTZone==null && (historyReferer != null && historyReferer.indexOf("true") >= 0)){
       %>
        <script>
               window.location.href = "../emxLogin.jsp";
        </script>
       <%
    }
    else{
