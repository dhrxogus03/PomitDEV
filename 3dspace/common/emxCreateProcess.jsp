...
<%
    ...
    //reading objectId and parentOID from createBean as hacker is able to inject it through request.
    HashMap formMap = createBean.getFormData(timeStamp);
    HashMap myRequestMap=(HashMap)formMap.get("requestMap");
    String objectId=(String) myRequestMap.get("objectId");
    if (UIUtil.isNullOrEmpty(objectId) || "undefined".equalsIgnoreCase(objectId))
    {
    	objectId = emxGetParameter(request, "objectId");
    }

  	String parentOID = (String) myRequestMap.get("parentOID");
	if (UIUtil.isNullOrEmpty(parentOID) || "undefined".equalsIgnoreCase(parentOID)) {
...
if(UIUtil.isNotNullAndNotEmpty(objectId) && !"undefined".equalsIgnoreCase(objectId)){
	String[] objectIdArray = {objectId};
	requestValuesMap.put("objectId",objectIdArray);
}

if(UIUtil.isNotNullAndNotEmpty(parentOID) && !"undefined".equalsIgnoreCase(objectId)){
	String[] parentOIDArray = {parentOID};
	requestValuesMap.put("parentOID",parentOIDArray);
}
    ...
    try {
        //added condition to handle the postProcessURL parameter by Ranganath
            if (postProcessURL != null && !"".equals(postProcessURL) && updateComplete) {
                ...
                %>
                <!-- //XSSOK -->
                <jsp:include page="<%=postProcessURL%>" flush="true"><jsp:param name="objectId" value="<%=objectId%>"/><jsp:param name="parentOID" value="<%=parentOID%>"/><jsp:param name="newObjectId" value="<%=newObjectId%>"/><jsp:param name="relId" value="<%=relId%>"/></jsp:include>
                <%
                }
...
//                 Rethrow the root cause exception
            ...
            if (postProcessJPO != null && !"".equals(postProcessJPO) && postProcessJPO.indexOf(":") > 0 && updateComplete) {
                ...
                HashMap postProcessRequestMap=UINavigatorUtil.getRequestParameterMap(pageContext);
                postProcessRequestMap.put("objectId",objectId);
                postProcessRequestMap.put("parentOID",parentOID);
                programMap.put("requestMap", postProcessRequestMap);
...
<%
            ...
            if (errorMessage != null && errorMessage.length() > 0) {
            	if(errorCode.equals("1500789") || errorMessage.contains("Business object 'type name revision' not unique")){ 
