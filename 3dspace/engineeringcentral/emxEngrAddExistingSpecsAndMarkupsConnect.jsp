...
<%@page import="com.matrixone.apps.engineering.PartDefinition,java.util.HashMap,com.matrixone.apps.domain.util.ContextUtil" %>
...
<%	
	...
	if(selectedItems != null){
		...
		try{
		    if(relType.equalsIgnoreCase("relationship_Markup")){
...
<%@include file = "emxDesignBottomInclude.inc"%>
		...
		if('true' == "<xss:encodeForJavaScript><%=addExistingSpecification%></xss:encodeForJavaScript>")
			...
			if(typeof getTopWindow().SnN != 'undefined' && getTopWindow().SnN.FROM_SNN){
				var frameWindow = findFrame(getTopWindow(),"<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>");
				findFrame(getTopWindow(),"<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>").refreshSBTable(frameWindow.configuredTableName);
			}
			else{
