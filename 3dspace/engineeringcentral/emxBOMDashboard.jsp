...
<% 	
	...
	//MyENGView change
	String strAppendObjects = XSSUtil.encodeForJavaScript(context, EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.MyEngViewSubHeader.KPIDataObj"));
	String strAppendState = XSSUtil.encodeForJavaScript(context, EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.MyEngViewSubHeader.KPIDataState"));
	String strAppendAge = XSSUtil.encodeForJavaScript(context, EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.MyEngViewSubHeader.KPIDataAge"));
	String strAppendWeeks = XSSUtil.encodeForJavaScript(context, EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.MyEngViewSubHeader.KPIDataWks"));
	String strAppendSeverity = XSSUtil.encodeForJavaScript(context, EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.MyEngViewSubHeader.KPIDataSev"));
			...
			$(document).ready(function () {
				...
				//creating Drilldown Data for Parts
					if(typeof(prevResponse) == 'undefined' || prevResponse == null || prevResponse == ''){
						...
						var stateDataEn = resultMap.split(",")[0].split("=")[1].replace(/-/gi, ",");
						var qtyPart = resultMap.substring(resultMap.indexOf("["), resultMap.lastIndexOf("]")+1).replace(/-/gi, ",");
						var stateData = resultMap.substring(resultMap.lastIndexOf("=")+1, resultMap.lastIndexOf("}")).replace(/-/gi, ",");
					...
					statesData.push({
						...
						events: { 
							click: function () {
								...
								subHeader = createSubheader(displayQty, totalStateQty, stateValue, null, null);
						...
						ages[index].push({ 
											...
											events: { 
												click: function () {
													...
													subHeader = createSubheader(displayQty, ageValue, stateValue, +i, null);
			            ...
			            column: {
					        ...
					        point: {
								events:{ click : function() {
									...
									subHeader = createSubheader(displayQty, +this.y, this.category, null, this.series.name);
			        ...
			        plotOptions: {
			            column: {
					        ...
					        point: {
								events:{ click : function() {
									displayQty = (+this.y > maxLimit) ? maxLimit : +this.y;
									subHeader = createSubheader(displayQty, +this.y, this.category, null, null);
...
function createSubheader(displayQty, totalQty, strState, strAge, strSeverity){
	...
	if( sHeader && sHeader.indexOf("|") > -1) {
			sHeader =sHeader.replace(/\|/g, encodeURIComponent("|"));
		}
	return sHeader;
}
