...
if ((bCallSubmit=="") || (bReload==null)){
    ...
    sURL = "../common/emxCreate.jsp?fromPartProperties=true&submitAction=doNothing&nameField=both&header=emxEngineeringCentral.Part.ClonePart&form=ENCClonePart&suiteKey=EngineeringCentral&targetLocation=slidein&multiPartCreation=true&HelpMarker=emxhelppartclone&copyObjectId=<%=XSSUtil.encodeForJavaScript(context,objectId)%>&type=<xss:encodeForJavaScript><%=selectedObjectType%></xss:encodeForJavaScript>&postProcessJPO=emxPart:postProcessForClonePart&preProcessJavaScript=preProcessInCreatePartClone&TypeActual=Part&createJPO=emxPart:checkLicenseAndCloneObject&partFamilyID=<xss:encodeForJavaScript><%=pfId%></xss:encodeForJavaScript>&PartFamilyAutoName=<xss:encodeForJavaScript><%=pfAutoNameGenerator%></xss:encodeForJavaScript>&postProcessURL=../engineeringcentral/PartCreatePostProcess.jsp?mode=clonedPartOpenInEditMode&emxTableRowId=<%=XSSUtil.encodeForJavaScript(context,objectId)%>&fromPartFamilyNav=" + isFromPartFamilyNav + "&slideinType=wider";
   //XSSOK 
