...
String fnLength  =   EnoviaResourceBundle.getProperty(context,"emxBOMPartManagement.FindNumberLength");
			 ...
			 if(((rdRequired && rdRequired.toLowerCase()=="true") || (rdRequired && rdQtyValidation && (rdRequired.toLowerCase()=="false"))) && (rdQtyValidation.toLowerCase()=="true"))
			  ...
			  qtyvalue = Math.round(qtyvalue);
				  if (qtyvalue != rdQty)
				  {
					...
					var sMsg="<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.ReferenceDesignator.SingleQuantity</emxUtil:i18nScript>";
					return sMsg;
				  }
			  }
		 }
...
function crossHighlightENG3DPlay(rowIds, flag) {
	...
	if(typeof channel3dPlayFrame != 'undefined' && channel3dPlayFrame != null) {
		...
		try {
				if(rowId == "0" && chkAllFlag ==  true && flag == false){
					throw " only root node enabled";
				}
				if(getCurrentValue("Quantity",rowNode) != null) {
					var quan = getCurrentValue("Quantity",rowNode);
					if(parseInt(quan)>1){
...
function createPart()
{
	...
	var strdata= encodeURIComponent(emxTableRowId);
	getTopWindow().showSlideInDialog("../engineeringcentral/PartCreatePreProcess.jsp?CreateMode=EBOM&multiPartCreation=true&selection=single&suiteKey=EngineeringCentral&objectId="+objId+"&emxTableRowId="+strdata,true,"","",550);
...
function addExistingPart()
{
	var listHidden = document.getElementById("listHidden");
 	...
 	var objId = (emxTableRowId.split("|"))[1];
	var strdata= encodeURIComponent(emxTableRowId);
 	var resultsPageURL = "../engineeringcentral/emxEngrFullSearchPreProcess.jsp?calledMethod=addExisting&suiteKey=EngineeringCentral&ENCBillOfMaterialsViewCustomFilter=engineering&objectId="+objId+"&emxTableRowId="+strdata;
...
function replaceExistingPart()
{
	...
	var strdata= encodeURIComponent(emxTableRowId);
 	var resultsPageURL = "../engineeringcentral/emxEngrFullSearchPreProcess.jsp?calledMethod=replaceExisting&suiteKey=EngineeringCentral&ENCBillOfMaterialsViewCustomFilter=engineering&objectId="+objId+"&emxTableRowId="+strdata;
...
function replaceNewPart()
{
	var listHidden = document.getElementById("listHidden");
 	...
 	var objId = (emxTableRowId.split("|"))[2];
	var strdata= encodeURIComponent(emxTableRowId);
 	var resultsPageURL = "../engineeringcentral/PartCreatePreProcess.jsp?CreateMode=EBOMReplaceNew&suiteKey=EngineeringCentral&ENCBillOfMaterialsViewCustomFilter=engineering&objectId="+objId+"&emxTableRowId="+strdata;
