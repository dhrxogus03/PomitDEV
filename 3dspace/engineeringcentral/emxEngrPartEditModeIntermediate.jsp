...
<%
	...
	try {
		...
		String objectSelectable = EngineeringConstants.SELECT_FROM_LEFTBRACE.concat(relationshipName).concat(EngineeringConstants.SELECT_RIGHTBRACE);
         ...
         String isConnectedToAnotherPart = partObject.getInfo(context, "relationship[EBOM]");
		...
		.append(csrfTokenName).append("=").append(csrfTokenValue);
		 ...
		 if(EngineeringConstants.TRUE.equalsIgnoreCase(isTemplateApplied))
	     {
			...
			*/
			...
			if(UIUtil.isNotNullAndNotEmpty(strChangePhyId)){
			DomainObject  controllingChange = DomainObject.newInstance(context,strChangePhyId);
			if(controllingChange.isKindOf(context, PropertyUtil.getSchemaProperty(context, "type_ChangeActionGroup"))){
				strURL.append("&changeBelongsToGroup=true");
			}
			}
	     }
		...
		strURL.append("&fromEditPartPage=true&postProcessURL=../engineeringcentral/emxEngrPartPortalRefresh.jsp&cancelProcessURL=../engineeringcentral/emxEngrPartPortalRefresh.jsp");
		 ...
		 if("true".equalsIgnoreCase(isConnectedToAnotherPart))
			 strURL.append("&sDisableSparePartYesOption=true");			 
