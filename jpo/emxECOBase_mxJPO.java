...
//    	if(ReleasePhase.isECPartWithDevMode(context, sPartId))
    	DomainObject domNewObj = DomainObject.newInstance(context, sPartId);
    	if( domNewObj.isKindOf(context, PartMgtConstants.TYPE_PART)&& !IPartLifecycleValidator.getService(context, sPartId).isChangeRequiredForRelease(context) ){
    		return 0;
    	}
        //BGP: In case the Release Process is Development, Do not execute the trigger program functionality
           ...
           // relPattern = relPattern; 
			...
			Iterator objItr3 = realizedchangeActionList.iterator();
			 ...
			 while(objItr2.hasNext())
             {
             	...
             	if(strCurrent.equals(ChangeConstants.STATE_CHANGE_ACTION_COMPLETE) || strCurrent.equals(ChangeConstants.STATE_CHANGE_ACTION_CANCEL)){
            ...
            ContextUtil.startTransaction(context, true);
                    ...
                    while(objItr1.hasNext())
                    {
                    	...
                    	if(current.equals(ChangeConstants.STATE_CHANGE_ACTION_COMPLETE) || current.equals(ChangeConstants.STATE_CHANGE_ACTION_CANCEL) || current.equals(EngineeringConstants.STATE_ECO_RELEASE) || current.equals(EngineeringConstants.STATE_ECO_IMPLEMENTED)){
                    		boolChangeCompleted = true;
                    		break;
                    	}
                    }
   ...
   */
   ...
   public Map createECO(Context context, String[] args) throws Exception{
			...
			try {
				...
				String strPartId = eco.createECO(context, sType, context.getVault().getName(), sPolicy, bAutoName);
