...
public class IEFBuildFolderStructure_mxJPO
{
	...
	{ 
		...
		{
	  		if(args[i].startsWith("IsNavigationInSaveNewDocument"))
	  		{
	  			StringTokenizer stringtokenizer = new StringTokenizer(args[i], "|");
	        	String key = stringtokenizer.nextToken();
	        	isNavigationInSaveNewDocument= stringtokenizer.nextToken();
	  		}
		}
		...
		try 
		{
			...
			if(generateDataFromCommand) 
			{
				...
				{
					String[] newArgs = {args[0], commandName, busId, sRelationshipName,isNavigationInSaveNewDocument,datetimepattern,locale,timezoneoffset};
					xmlOutPut = evaluateUIComponent(context, newArgs);
				}
				else
				{
					String[] newArgs = {args[0], commandName, busId, sRelationshipName,isNavigationInSaveNewDocument};
					xmlOutPut = evaluateUIComponent(context, newArgs);
				}
				generateDataFromCommand = true;
			}
...
//issue related to multiple name display of image file
			...
			{               
				...
				if(!notAllowedAttributes.contains(attributeName))
				{
					...
					if(attributeName != null && attributeName.startsWith("XP_") && attributeName.contains(".")) {
						...
						adminName = strArray[strArray.length - 1];
					...
					//IR-1185547 , IR-975287 : If attribute created using attribute mgmt then in text should contain attribute name with interface.
					if(attributeName != null && attributeName.startsWith("XP_") && attributeName.contains(".") && adminName.equals(displayAttributeName)) {
						String text  = "emxFramework." + "Attribute" + "." + attributeName.replace(' ','_');
				        ...
				        try{
				        	...
				        	System.out.println("I18NreturnString = "+I18NreturnString  );
				        	if ((!"".equals(I18NreturnString)) && (I18NreturnString != null) && !(I18NreturnString.equals(text))){
				        		displayAttributeName = I18NreturnString;
				            }
				        }
				        catch(Exception e)
				        {
				            ...
				            System.out.println("[IEFBuildFolderStructure.getAttributeList] EXCEPTION : " + text + " not found in emxFrameworkStringResource");
				        }
					}
	...
	{ 
		...
		try 
		{
            ...
            String[] newArgs = {args[0], symbolicCommandName, busId, null,null,datetimepattern,locale,timezoneoffset};
			xmlOutput = evaluateUIComponent(context, newArgs);
		}
		catch (Exception ex)
		{
