 ...
 * intended for use within jsp pages.
  * @version Copyright (c) 1992-2020 Dassault Systemes.
 */
...
public class enoBGTPManagerBase_mxJPO  extends emxCommonPart_mxJPO {
	...
	public void updateChangeControlled(Context context, String[] args) throws Exception {
		...
		if(workUnderOID!=null && workUnderOID.length>0){
			 workUnderId=workUnderOID[0];
		}
		
		String [] changeControlled = (String[])requestMap.get("ChangeControlled");
		String objectId = (String)paramMap.get("objectId");
		
		if(!UIUtil.isNotNullAndNotEmpty(workUnderId)){
			boolean boolChangeControlled = (changeControlled != null && changeControlled.length>0 && "true".equals(changeControlled[0]));
			
			IPartService iPartService = IPartService.getService(context, objectId);
			iPartService.setChangeControlled(context, boolChangeControlled);
		}
	...
	public String getConfiguredOption(Context context,String[] args) throws Exception {
		...
		if(EngineeringUtil.isVPMInstalled(context, args) && SynchronizationUtil.isOCDXUser(context) ) {
			sbHTMLOutPut.append("<tr>");
			sbHTMLOutPut.append("<td>");
			//sbHTMLOutPut.append("<span>");
	...
	throws Exception {
		...
		HashMap paramMap = (HashMap)JPO.unpackArgs(args);
        String objectId      = (String) paramMap.get("objectId");
        Map settingsMap = (Map)paramMap.get("SETTINGS");
        
		String objectIds      = UIUtil.isNotNullAndNotEmpty((String) paramMap.get("objectIds"))?(String) paramMap.get("objectIds"):(String) paramMap.get("strObjectIds");
		
		if ( UIUtil.isNullOrEmpty(objectIds) ) { objectIds = objectId; }
		
		if(UIUtil.isNotNullAndNotEmpty(objectIds)){
			String[] selObjectIds = objectIds.split("\\|");
			
			boolean ifChangeRequired = true;
			
			for(int i=0;i<selObjectIds.length;i++){
				String strObj = selObjectIds[i];
				IPartService iPartService = IPartService.getService(context, strObj);
		        ifChangeRequired = iPartService.isChangeControlled(context);
		        if (ifChangeRequired) { break;}
			}
			
			if (!ifChangeRequired) {
				settingsMap.put("Required", "false");
			}
		}
	...
	public void addChange(Context context, String args[]) throws Exception
	{
		...
		if(affetctedItemsIDs.size()>0) {
			...
			if(BOMMgtUtil.isNullOrEmpty(sCAId) && BOMMgtUtil.isNotNullAndNotEmpty(sAlreadyConnectedCAId)){
				sCAId = sAlreadyConnectedCAId;
			}
			...
			for(int i=0; i<iProposedChanges.size();i++){
				...
				if(affetctedItemsIDs.contains(sObjectId)) {
						IBusinessObjectOrRelationshipObject  iBusinessObjectOrRelationshipObject=ProposedActivityFactory.CreateIProposedActivityArgument(new BusinessObject(sObjectPhyId));
				...
				//Add all affecteditems to CR per new ECM process. And add CA to CR as Implementing Change
				if(changeObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST))
				{
					EngineeringUtil.createConnectionsWithCAandCR(context, changeOID, sCAId, affetctedItemsIDs);
				}
		}
	...
	public com.matrixone.jdom.Element createXMLForItemMarkUp(Context context, DomainObject doPartObj, DomainObject doMarkup) throws Exception
	{
		...
		attNameElement.setText(EngineeringConstants.ATTRIBUTE_CHANGE_CONTROLLED);
		 String objectId=doPartObj.getObjectId(context);
         
         IPartService ipart=IPartService.getService(context, objectId);
		 attOldElement.setText(ipart.isChangeControlled(context) ? "Enabled" : "Disabled");
		 
		attNewElement.setText("Enabled");
		attElement.addContent(attNameElement);
		attElement.addContent(attOldElement);
		attElement.addContent(attNewElement);
		rootElement.addContent(attElement);
	...
	public boolean accessUnderFormalChangeCommand(Context context, String[] args) throws Exception{
		...
		/*
		 Added for GLS team as a temp fix, check if the type is dervived from Product Data and if yes hide ENG push to production command - start 
		*/
		
		String objectType = partObj.getInfo(context, DomainConstants.SELECT_TYPE);
		
		String TYPE_PRODUCT_DATA = PropertyUtil.getSchemaProperty(context, "type_ProductDataPart");
		
		if ( ! UINavigatorUtil.isCloud(context) && UIUtil.isNotNullAndNotEmpty(TYPE_PRODUCT_DATA) ) {
			if ( mxType.isOfParentType(context, objectType, TYPE_PRODUCT_DATA) ) {
				return false;
			}
		}
		
		/*
		 Added for GLS team as a temp fix, check if the type is dervived from Product Data and if yes hide ENG push to production command - End 
		 */
	...
	public StringList allowEditInMulitpartCreatePage(Context context, String[] args) throws Exception
	{
		...
		for(int i =0; i<mlObjectList.size();i++){
			...
			slReturnList.add( String.valueOf( !( EngineeringConstants.MANDATORY.equalsIgnoreCase(changeControlledReqd) || EngineeringConstants.POLICY_CONFIGURED_PART.equals(sPolicy) ) ) );
		}
		return slReturnList;
	}
	...
	public int checkIfPartHasItemMarkupNotApplied(Context context, String []args)  throws Exception {
		...
		final String RELATIONSHIP_APPLIED_MARKUP =PropertyUtil.getSchemaProperty("relationship_AppliedMarkup");
		...
		objSelects.add("to["+RELATIONSHIP_APPLIED_MARKUP+"].from."+DomainConstants.SELECT_CURRENT); 
		...
		for(int i = 0; i < sMarkupIDState.size(); i++)
		{
			...
			String strChangeState =(String) map.get("to["+RELATIONSHIP_APPLIED_MARKUP+"].from."+DomainConstants.SELECT_CURRENT);
			boolean isBGTPMarkup = ReleasePhase.isItemMarkupForSetToProduction(context,sMarkupId);

			if(isBGTPMarkup && !sMarkupCurrent.equals(sMarkupState)&& !ChangeConstants.CANCELLED.equalsIgnoreCase(strChangeState))
			{
	...
	/**
	 * Access method for Change Required(Instance) columns. 
	 * Returns true if expression ECM_EnableChangeControlGranularity is enabled
	 * @param context
	 * @param args
	 * @return boolean
	 * @throws Exception
	 */
	public boolean displayChangeRequiredInstanceColumn(Context context, String[] args) throws Exception{
		boolean boolReturn = false;

		if(UIUtil.isNotNullAndNotEmpty(MqlUtil.mqlCommand(context, "list expression $1","ECM_EnableChangeControlGranularity"))){
			String sExpr = MqlUtil.mqlCommand(context, "print expression $1 select $2 dump", "ECM_EnableChangeControlGranularity", "value");
			if(UIUtil.isNotNullAndNotEmpty(sExpr) && "true".equalsIgnoreCase(sExpr))
				boolReturn = true;
		}
		return boolReturn;
	}
