			...
			for(int i = 0; i < objectList.size();i++){
				...
				if(EngineeringUtil.isPartAuthorizedToHostCompany(context, strMEPId)){
					...
					if(!TYPE_MPN.equals(mepAltSubInfo.get(2))) {
						...
						if("True".equalsIgnoreCase(mepAltSubInfo.get(0))) 
							mepUsage = ALTERNATE;

						else if("True".equalsIgnoreCase(mepAltSubInfo.get(1))) 
							mepUsage = SUBSTITUTE;
		...
		try{
			...
			if("DefineAlternate".equalsIgnoreCase(sAction)) {
				...
				for (int i = 0; i < slParentId.length; i++) {
					...
					//Added to avoid warning messages while doing setAttributeValues due to non existence of some attributes
					String mqlString="list relationship $1 select $2 dump $3";
		            StringList attributeList = FrameworkUtil.split(MqlUtil.mqlCommand(context,mqlString,RELATIONSHIP_EBOM_SUBSTITUTE,"attribute","|"), "|");
		                        
					AttributeList attList = (new DomainRelationship(sEBOMRelId)).getAttributeValues(context, attributeList);
					...
					//Set the attributes that exist on EBOM Substitute rel
					(new DomainRelationship(sEBOMSubRelId)).setAttributeValues(context, FrameworkUtil.toMap(attList));
				...
				for (int i = 0; i < slParentId.length; i++) {
					...
					if("False".equalsIgnoreCase(sEBOMSubstitute)){
						connectAlternatePart(context, sMEPId, strPrimaryPartId);
					}
				}
			}
