...
public class emxCommonFindObjectsBase_mxJPO
{
    ...
    public void getIds(Context context, int chunkSize) throws Exception
    {
        ...
        try
        {
			String finalWhereClause = " program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true";
            if( isType ) {
				if(hasWhereExp){
					finalWhereClause = "(" + whereExp +") && " + finalWhereClause;
				}
				MqlUtil.mqlCommand(context,"temp query bus $1 $2 $3 vault $4 limit $5 where $6",type,name,revision, "*", "1",finalWhereClause);
            } else {
