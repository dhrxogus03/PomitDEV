...
public class emxBOMPartManagementBase_mxJPO extends emxCommonPart_mxJPO {
	...
	public static Map getAssociatedChange(final Context context, final String objectId, StringList requestedChange,boolean includeRequestedchange) throws MatrixException {
		Map<String, String> mAssociatedChanges= new HashMap<String,String>();
		try {			
			IChangeActionServices iChangeActionServices = ECMUtil.getChangeActionServicesInstance();
			
			String physicalId = PartMgtKernel.getPhysicalId(context, objectId);
			
			Map < String, Map < IChangeAction, List < IOperation > > > operationListMap = iChangeActionServices.getProposedOperationAndChangeActionFromIdList( context, new StringList(physicalId) );
			Map < IChangeAction, List < IOperation > > operationMap = operationListMap.get(physicalId);
			
			if (operationMap != null) {
				Iterator iterator = operationMap.keySet().iterator();				
				IChangeAction iChangeAction;				
				IOperation iOperation;
				
				while ( iterator.hasNext() ) {
					iChangeAction = (IChangeAction) iterator.next();
					
					if ( !iChangeAction.isCancelled(context)) {					
						List <IOperation> ioperationList = (List) operationMap.get(iChangeAction);
						
						for (int i = 0; i < ioperationList.size(); i++) {
							iOperation = ioperationList.get(i);						
							
							String strRequestedChange = ChangeAction.getRequestedChangeFromOperationAndTargetedState( context, iOperation.getOperationName(), iOperation.getTargetStatus() );
							
							BusinessObjectWithSelect businessObjectWithSelect = iChangeAction.getChangeOrder(context);							
							String sChangeID = (businessObjectWithSelect != null) ? businessObjectWithSelect.getSelectData(PartMgtConstants.SELECT_PHYSICAL_ID) : null;
							
							if ( strRequestedChange!=null ){							
							  if((includeRequestedchange  && requestedChange.contains(strRequestedChange)) || (!includeRequestedchange  && ! (requestedChange.contains(strRequestedChange)))) {
								  if(UIUtil.isNotNullAndNotEmpty(sChangeID))
								    mAssociatedChanges.put(sChangeID,strRequestedChange);
								  else {
									  BusinessObject busChangeAction =  iChangeAction.getCaBusinessObject();
									  mAssociatedChanges.put(busChangeAction.getObjectId(),strRequestedChange);
								  }
							  }							
						    }
						}
			...
			lBusoBject.add(new BusinessObject(objectId));
			Map<String, Map<IChangeAction, List<IRealizedChange>>> realizedMap = iChangeActionServices.getRealizedAndCaFromListObjects( context, lBusoBject,false,true,true);
						
			for(Entry<String, Map<IChangeAction,List<IRealizedChange>>> mapOutput : realizedMap.entrySet()){
				for(Entry<IChangeAction,List<IRealizedChange>> mapOutput2: mapOutput.getValue().entrySet()){
					...
					if(iChangeAction!= null){
						...
						String strRequestedChange = new ChangeAction().getReaquestedChangeForRealizedFromChangeAction(context, objectId, strChangeActionID);
							...
							if((includeRequestedchange  && requestedChange.contains(strRequestedChange)) || (!includeRequestedchange  && ! (requestedChange.contains(strRequestedChange)))) {
								  if(UIUtil.isNotNullAndNotEmpty(sChangeID))
								    mAssociatedChanges.put(sChangeID,strRequestedChange);
									  ...
									  mAssociatedChanges.put(strChangeActionID,strRequestedChange);
								  }
							}
						}
				    }
		...
		catch (Exception ex) { throw new MatrixException(ex); }
		
		return mAssociatedChanges;
	}
    ...
    {
    	// RPE variable for triggers to identify invoke is due to floatEBOMToEnd operation (emxBOMPartManagementBase.floatEBOMToEnd ())
        // Used to hanlde dead-lock situation in MCM triggers in MCM-ENG installed env when multiple CO accepted 
        PropertyUtil.setGlobalRPEValue(context, "FLOAT_EBOM_TO_END", "TRUE");
    ...
    /*if(UIUtil.isNullOrEmpty(sChangeOrder)){// throw new Exception();
    	sChangeOrder = getIncompleteProposedCOId(context, selectStmt,objectId);
	 }*/
	 
	Map<String, String> mAssociatedChange=getAssociatedChange(context, objectId, new StringList("For Release"), true);
    String sContextPartAssociatedChange=null;    		 
		
    for (Entry mAssociatedChangePairs : mAssociatedChange.entrySet()) {
    	    		sContextPartAssociatedChange = (String) mAssociatedChangePairs.getKey();
	 } 	
	...
	if (!(rel_SymbolicName != null && rel_SymbolicName.equals("relationship_ManufacturerEquivalent"))){
	    ...
	    String sPrevRevPartState=prevRevPart.getInfo(context, DomainConstants.SELECT_CURRENT);
        ...
        createBOMHistoryRel(context, strRelationName, (String)m.get(DomainConstants.SELECT_ID), strPreviousId, attributeMap);
		    ...
		    if(isGBOM){
		    	...
		    	if("True".equalsIgnoreCase(sFloatToOlderRevision)){
					...
					Map<String,String> mParentPartAssociatedChange= getAssociatedChange(context, strPartId, new StringList("For Release"), true);
		    		String sParentPartAssociatedChange="";
		    		for (Entry mParentPartAssociatedChangePairs : mParentPartAssociatedChange.entrySet()) {
		    		     sParentPartAssociatedChange = (String) mParentPartAssociatedChangePairs.getKey();
		    		 }
                    ...
                    Map<String,String> mParentPartNextRevAssociatedChange=null;
		    		String sParentPartNextRevAssociatedChnage="";
		    		if(UIUtil.isNotNullAndNotEmpty(strNextRevId)) {
		    		   mParentPartNextRevAssociatedChange= getAssociatedChange(context, strNextRevId, new StringList("For Release"), true);
		    		   for (Entry mParentPartNextRevAssociatedChangePairs : mParentPartNextRevAssociatedChange.entrySet()) {
		    		    	sParentPartNextRevAssociatedChnage = (String) mParentPartNextRevAssociatedChangePairs.getKey();
		    		    }
		    		 }
		    		...
		    		if((releaseState.equalsIgnoreCase(sCurrentState) && !(sContextPartAssociatedChange.equalsIgnoreCase(sParentPartNextRevAssociatedChnage))) || (PolicyUtil.checkState(context, strPartId, obsoleteState,PolicyUtil.LT) && !(sContextPartAssociatedChange.equalsIgnoreCase(sParentPartNextRevAssociatedChnage)) && sContextPartAssociatedChange.equalsIgnoreCase(sParentPartAssociatedChange))){
		    			DomainRelationship.setToObject(context, (String)m.get(DomainConstants.SELECT_RELATIONSHIP_ID), this);
		    		...
		    		if(obsoleteState.equalsIgnoreCase(sPrevRevPartState))
		    			DomainRelationship.setAttributeValue(context, (String)m.get(DomainConstants.SELECT_RELATIONSHIP_ID), DomainConstants.ATTRIBUTE_END_EFFECTIVITY_DATE, "");
	...
	public void createBOMHistory(Context context,String parentId,String childId, Map relAttr, String historyRelName,String excludeAttr) throws Exception {
        ...
        if (UIUtil.isNullOrEmpty((String)relAttr.get(DomainConstants.ATTRIBUTE_END_EFFECTIVITY_DATE))){
        	SimpleDateFormat _mxDateFormat = new SimpleDateFormat(eMatrixDateFormat.getEMatrixDateFormat(), Locale.US);
        	relAttr.put(DomainConstants.ATTRIBUTE_END_EFFECTIVITY_DATE,_mxDateFormat.format( Calendar.getInstance().getTime()));
        }
...
public int checkForPartObsolecense(Context context,String args[]) throws Exception {
    ...
    {   
	    ...
	    if( !mlBOMParents.isEmpty() && mlBOMParents.size() > 0) {
	    		String errMsg = EnoviaResourceBundle.getProperty(context, "emxBOMPartManagementStringResource", context.getLocale(),"emxBOMPartManagement.Part.PartObsoleteMsg");
