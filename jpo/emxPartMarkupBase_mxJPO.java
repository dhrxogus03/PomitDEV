...
public class emxPartMarkupBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public static final String ATTRIBUTE_LEAD_TIME=PropertyUtil.getSchemaProperty("attribute_LeadTime");
    public static final String ATTRIBUTE_LEAD_TIME_DURATION=PropertyUtil.getSchemaProperty("attribute_LeadTimeDuration");
    public static final String ATTRIBUTE_TARGET_COST=PropertyUtil.getSchemaProperty("attribute_TargetCost");
  ...
  public static final String RELATIONSHIP_DERIVED = PropertyUtil.getSchemaProperty("relationship_Derived");//added for bug 360985
	...
	public final static String MARKUP_VALIDATOR = "Markup";
	public static final String POLICY_REQUEST_FOR_CHANGE = PropertyUtil.getSchemaProperty("policy_RequestForChange");
    public static final String STATE_CHANGEREQUEST_APPROVED = PropertyUtil.getSchemaProperty("policy", POLICY_REQUEST_FOR_CHANGE, "state_Approved");
    public static final String RELATIONSHIP_CHANGE_IMPLEMENTATION = PropertyUtil.getSchemaProperty("relationship_ChangeImplementation");
    ...
    public static String mergeMarkup(Context context, String[] args ) throws Exception
    {
      ...
      //Modified for ENG Convergence start
        ...
        if(UIUtil.isNotNullAndNotEmpty(strMode) && "mergeFromPartContext".equals(strMode)) {
        	strChangeId = UIUtil.isNotNullAndNotEmpty((String)paramMap.get("COId"))?(String)paramMap.get("COId"):(String)chhId.get(0);          	 
        } else {
...
/**
    ...
    public MapList getAllMarkupSummary(Context context,String[] args)throws Exception
    {
      ...
      public int applyMarkupOnChange(Context context,String[] args) throws Exception
      {
         ...
         String stre="true";
		 ContextUtil.pushContext(context,"","",context.getVault().getName());  
         ...
         try
         {
			...
			StringList slMultiValueSeletables = new StringList(1);
			slMultiValueSeletables.addElement("from["+RELATIONSHIP_EBOM+"].to.id");
			...
			if (doChange.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)){
				...
				mapListParts = EngineeringUtil.getProposedItems(context, changeId, strlObjectSelects, slMultiValueSeletables);
			}
			else {
				mapListParts = doChange.getRelatedObjects(context,
						strAIRelPattern, // relationship pattern
			...
			while(itrParts.hasNext())
			{
	            ...
	            if(null != objectData){
	    			if (objectData instanceof StringList) {
	    				...
	    				slEBOMList.addElement((String) objectData);
	    			}
	            }
	            if(slEBOMList != null){
    ...
    public static int checkForUniqueness(Context context, Element eleRoot, List fnVals, List refDesVals)
      ...
      public static int applyBOMMarkup(Context context,String strMarkupId,String strPartId, String strChangeId) throws Exception
      {
          ...
          String sChangeId = UIUtil.isNotNullAndNotEmpty(strChangeId)? strChangeId: strPartId;
          if(UIUtil.isNotNullAndNotEmpty(sChangeId)){
        	  DomainObject doChgObj = new DomainObject(sChangeId);
        	  if(doChgObj.isKindOf(context, TYPE_PART)){
        		  changeActionId = PartMgtUtil.getWorkUnderChangeId(context, strPartId);
        	  }
        	  else if(doChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)){
        		  changeActionId = sChangeId;
        	  }
          }
      String strCRPhysicalId = new DomainObject(changeActionId).getInfo(context, "to["+ RELATIONSHIP_CHANGE_IMPLEMENTATION +"].from.physicalid");
    if(UIUtil.isNotNullAndNotEmpty(strCRPhysicalId)) {
     IChangeRequest iCR = EngineeringUtil.getChangeRequestFromService(context, strCRPhysicalId);
     if(!iCR.isApproved(context)) {
    	 String errorMessage = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.ApplyMarkup.ErrorForApprovedCR");
        ...
        //Multitenant
         ...
         String strisVPMVisible =  PropertyUtil.getSchemaProperty(context,"attribute_isVPMVisible");
          ...
          if ("true".equals(fnUniqueness) || "true".equals(rdUniqueness)) {
             ...
             /* if(flag==1){
                  ${CLASS:emxContextUtil}.mqlNotice(context,strFNMsg);
                  return 1;
              }else*/
              if(flag==2){
          ...
          StringList slSortedList = new StringList();
   		  StringList slModifiedList = new StringList();
          while (itrRelationships.hasNext())
          {
			 ...
			 strPartVault= com.matrixone.apps.domain.util.XSSUtil.decodeFromURL(eleObjectInfo.getChild("vaultRef").getText());
              ...
              attrEBOMChgType = eleRelatioship.getAttribute("chgtype");
              connectedrelID = (eleRelationshipRefDef.getAttribute("relid")).getValue();
              ...
              if ("A".equals(strEBOMChgType))
              {
                  ...
                  //HF-050350 Ends
				   ...
				   if(strCurrent.equals(DomainObject.STATE_PART_OBSOLETE)){
					   ...
					   if(!strParentState.equals(DomainObject.STATE_PART_PRELIMINARY)) {
	...
	//                          HF-056391 Starts
                       ...
                       if (strCurrent.equals(DomainObject.STATE_PART_OBSOLETE) && strLastRevCurrent.equals(DomainObject.STATE_PART_RELEASE))
                  {
                      ...
                      if(blnIsEBOMSub) {
                    	...
                    	String parentId = new DomainObject(doParentPart).getObjectId(context);
                      	...
                      	iBOMIngress.setBOMUI(MARKUP_VALIDATOR); 
						
                      	IBOMService iBOMService = IBOMService.getService(context, parentId, false);
                		iBOMService.setAuthoringChange(changeActionId);
                  ...
                  else
                  {
                      ...
                      if(blnIsEBOMSub) {
                          ...
                          doRelNew.setAttributeValues(context, hmpAttributes);
                        	...
                        	iBOMIngress.setBOMAttributeMap(hmpAttributes);
                        	iBOMIngress.setBOMUI(MARKUP_VALIDATOR); 
                        	String sNextRelId = "";String sSelectedRelId = "";
                        	if(eleRelatioship.getChild("addNextElement") != null){
	                        	sNextRelId = eleRelatioship.getChild("addNextElement").getChild("nextElemet").getText();
	                        	sSelectedRelId = eleRelatioship.getChild("addNextElement").getChild("selectedElement").getText();
                        	}
                        	...
                        	iBOMService.add(context, iBOMIngress);
                        	StringList slInstanceIds =iBOMService.getInstanceIds();
                        	slModifiedList.addAll(slInstanceIds);
	                		
	                		if(PartMgtUtil.isNotNullAndNotEmpty(sSelectedRelId) && !slSortedList.contains(sSelectedRelId)){
	            				slSortedList.add(sSelectedRelId);
	    	        		}
	    	        		slSortedList.addAll(slModifiedList);
	    	        		if(PartMgtUtil.isNotNullAndNotEmpty(sNextRelId) && !slSortedList.contains(sNextRelId)){
	    	        				slSortedList.add(sNextRelId);
	    	        		}
              ...
              else if ("C".equals(strEBOMChgType))
              {
                  ...
                  //End - Added for EBOM Substitute Mass change
                      ...
                      if (mapListBOMs.size() > 0)
                      {
                          ...
                          IBOMService iBOMService = IBOMService.getService(context, strEBOMRelId, true);
                          iBOMService.setAuthoringChange(changeActionId);
                          iBOMService.setCollaboratToDesign(true);
              ...
              else if ("D".equals(strEBOMChgType))
              {
                  ...
                  //End - Added for EBOM Substitute Apply
					...
					String logicalIdOfRel= (String)((Map)DomainRelationship.getInfo(context, new String[] {connectedrelID}, new StringList("logicalid")).get(0)).get("logicalid");
                      if(UIUtil.isNotNullAndNotEmpty(strRelWhereClause))
                    	  strRelWhereClause += " && logicalid=="+logicalIdOfRel;
                      MapList mapListBOMs = doParentPart.getRelatedObjects(context,
                                              DomainConstants.RELATIONSHIP_EBOM,  // relationship pattern
                                              DomainConstants.TYPE_PART,          // object pattern
                                              strlPartSelects,                 // object selects
                      ...
                      if (mapListBOMs.size() > 0)
                      {
                		...
                		iBOMService.setAuthoringChange(changeActionId);
                		iBOMService.setCollaboratToDesign(true);
                    	iBOMService.remove(context, iBOMIngress);
          ...
          if(slSortedList != null && slSortedList.size()>0){
       		IBOMOrderService treeOrderImpl = IBOMOrderService.getService();
      		treeOrderImpl.persistOrder(context, slSortedList,slModifiedList);
   		}
...
//ADDED For BUG #347077
    ...
    public void applyingItemMarkup(Context context, String strMarkupId, String strPartId, String strChangeId) throws Exception
    {
    	 if(!ReleasePhaseManager.isItemMarkupForSetToProduction(context, strMarkupId)){
         	IPart iPart = IPartService.getService(context, strPartId).getInfo(context);
         	if(!(iPart.isApproved() && !iPart.isVPMVisible())){
    			if((!BOMMgtUtil.isModificationAllowed(context, iPart.getPolicy(), iPart.getCurrent(), "emxBOMPartManagement.ECPart.AllowApply"))){
    				String propertyKey = "emxBOMPartManagement.Validation.NonModifiablePartStateForMarkUp";
    				
    				String [] messageValues = new String[4];
    				messageValues[0] = BOMMgtUtil.getDisplayPropertyValue(context, "emxFramework.Type." + iPart.getType(), "emxFrameworkStringResource");
    	            messageValues[1] = iPart.getName();
    	            messageValues[2] = iPart.getRevision();
    	            messageValues[3] = BOMMgtUtil.getDisplayPropertyValue(context, "emxFramework.State." + iPart.getPolicy().replace(" ", "_") + "." + iPart.getCurrent(), "emxFrameworkStringResource");
    	
    				String errorMsg = BOMMgtUtil.getMacrosConvertedPropertyValue(context, propertyKey, messageValues, BOMMgtConstants.BOM_MGT_RESOURCE_FILE );
    				
    				throw BOMMgtUtil.createBOMException(propertyKey, errorMsg, errorMsg);
    			}
    		 }
         }
    	String changeActionId ="";
    	 String strCRPhysicalId ="";
        if(UIUtil.isNullOrEmpty(strChangeId)){
        	strCRPhysicalId = new DomainObject(strMarkupId).getInfo(context, "to["+RELATIONSHIP_APPLIED_MARKUP+"].from.to["+RELATIONSHIP_CHANGE_IMPLEMENTATION+"].from.physicalid");
      	}else {
      		strCRPhysicalId = new DomainObject(strChangeId).getInfo(context, "to["+RELATIONSHIP_CHANGE_IMPLEMENTATION+"].from.physicalid");
      	}
       if(UIUtil.isNotNullAndNotEmpty(strCRPhysicalId)) {
    	   IChangeRequest iCR = EngineeringUtil.getChangeRequestFromService(context, strCRPhysicalId);
    	   if(!iCR.isApproved(context)) {
        ...
        {
        	System.out.println("Exception in bus open" + context.getUser());
        	System.out.println(strMarkupId + strPartId + strChangeId);
        	mex.printStackTrace();
        	throw mex;
            //return;
        }
        ...
        {
        	System.out.println("Exception in Checkout file" + context.getUser());
        	System.out.println(strMarkupId + strPartId + strChangeId);
        	mex.printStackTrace();
        	throw mex;
            //return ;
        ...
        // get file values into file content.
	        while(attributeItr.hasNext())
	        {
	            ...
	            if(ATTRIBUTE_LEAD_TIME.equals(attName))
	            {	AttributeType attTypeLTD = new AttributeType(ATTRIBUTE_LEAD_TIME_DURATION);
	            	attTypeLTD.open(context);
	            	String strLTDDefaultValue=attTypeLTD.getDefaultValue();
	            	attTypeLTD.close(context);
                	attName = ATTRIBUTE_LEAD_TIME_DURATION;
                	if("Unassigned".equals(attNewValue)){
                		attNewValue = strLTDDefaultValue;
                	}else{
                		attNewValue = attNewValue.split(" ")[0]+" Week";
                	}
	            }
	                       
	            if((EngineeringConstants.ATTRIBUTE_CHANGE_CONTROLLED.equals(attName) && "Enabled".equalsIgnoreCase(attNewValue))||(EngineeringConstants.ATTRIBUTE_CHANGE_CONTROLLED.equals(attName) && "true".equalsIgnoreCase(attNewValue))){
	            IChangeControlled iChangeControl = IChangeControlled.getService(context, ECMUtil.getChangeControlledValue(context, strPartId));
	            	
	            	if(!iChangeControl.is_Strict()){	
						IPartService iPartService = IPartService.getService(context,strPartId);
						iPartService.setChangeControlled(context, Boolean.TRUE);
					}
	            }
	            else if (!ATTRIBUTE_LEAD_TIME.equals(attName))
    ...
    throws Exception
{
	...
	try{
          ...
          //Multitenant
            ...
            String sDevError = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.MarkupActions.ApplyDevMarkupError");
        ...
        while(stritr.hasNext())
        {
            ...
            }else if(POLICY_DEVELOPMENTPARTMARKUP.equals(currentpolicyName)){
            	emxContextUtil_mxJPO.mqlNotice(context,sDevError);
            }
       }
    }
catch(Exception ex)
    ...
    throws Exception
{
	...
	StringList iMKpartIdList = new StringList();
	//Multitenant
	String sConflictErr =EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.MarkupActions.ConflictError");
    ...
    try{
    	isMarkupConnectedToCancelledChnage(context, markupobjList);
    }catch(Exception e){
    	emxContextUtil_mxJPO.mqlNotice(context,e.getMessage());
    	return 1;
    }
...
if ("false".equals(sConflict))
{
  ...
  return 1;
               ...
               if(!DomainConstants.POLICY_DEVELOPMENT_PART.equals(strPartPolicy) &&(currentState.equals(approvedState)))
                {
					...
					boolean isApplyMarkUpAllowed = ((EngineeringConstants.POLICY_EC_PART.equals(strPartPolicy)) && (DomainConstants.STATE_PART_PRELIMINARY.equals(strPartState))) ? true : (strPartCurrentRev.equals(strPartCurrentFirstRev) && "FALSE".equals(strPartVer) && strPartState.equals(DomainConstants.STATE_PART_PRELIMINARY))  || strPartState.equals(DomainConstants.STATE_PART_REVIEW) || strPartState.equals(DomainConstants.STATE_PART_APPROVED) || TYPE_PLANT_BOM_MARKUP.equals(strMarkupType);
                        
					//R209 MECO changes - Starts
					if(isApplyMarkUpAllowed)
						...
						else
						{
							...
							else
							{
								...
								String [] mailArguments = new String [8];
								mailArguments[0] = ((TYPE_EBOMMARKUP.equals(strMarkupType) || TYPE_BOMMARKUP.equals(strMarkupType) || TYPE_ITEMMARKUP.equals(strMarkupType)))?"emxEngineeringCentral.Alert.InValidStateError3":"emxEngineeringCentral.Alert.InValidStateError2";
								mailArguments[1] = "2";
								mailArguments[2] = "strPartName";
								mailArguments[3] = strPartName;
								mailArguments[4] = "markupName";
    ...
    {
		collaboratePartAfterItemMarkApply(context, iMKpartIdList);
    }
    return 0;
}
    ...
    public void createMarkup(Context context,String args[]) throws Exception
    {
    	...
    	try {
			...
			StringList partDataMap = new StringList();
			partDataMap.add(PartMgtConstants.SELECT_CURRENT);
			partDataMap.add(PartMgtConstants.SELECT_POLICY);
			DomainObject doPart = new DomainObject(strPartId);
			
			Map objMap = doPart.getInfo(context, partDataMap);
				...
				if(doChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ORDER) || doChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST)) {
					...
					if("TRUE".equalsIgnoreCase(changeConnected)) {						//
						changeOID = (String)changeActionMap.get(ChangeConstants.RELATED_CA_ID);
					      ...
					      if(doChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST)) {
			          	  EngineeringUtil.createConnectionsWithCAandCR(context, changeOID, strCAId, new StringList(strPartId));
			            	} 
					      changeOID = strCAId;
					}
					String current = (String) objMap.get(DomainConstants.SELECT_CURRENT);
					if(EngineeringConstants.STATE_EC_PART_RELEASE.equals(current)){
						boolean isChangeActionBeyondInwork = EngineeringUtil.getChangeAction(context, changeOID).isInWorkOrMore(context);
						if(isChangeActionBeyondInwork){
							String strMessage = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.EBOM.MarkupSaveConnectErrorOnReleasedPart");
							emxContextUtil_mxJPO.mqlNotice(context, strMessage);
							return;
						}
					}
	        ...
	        if("Development".equalsIgnoreCase(policyClassification) || isDevModePart) {
				   ...
				   generateBOMMarkupXML(context,strArray);
	                ...
	                //  Create an object with User choosed Name
						...
						sdomobj.createObject(context,TYPE_BOMMARKUP,sName,sRevision,POLICY_PARTMARKUP,context.getVault().getName());
	            ...
	            // For EC Parts
	                ...
	                if(sCommandType==null || sCommandType.equalsIgnoreCase("Save")) {
						...
						if (BOMMgtUtil.isNotNullAndNotEmpty(changeOID)) {
	                    ...
	                    if(isChangeconnected.equalsIgnoreCase("true")) {
							...
							if(ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)) {
								...
								DomainRelationship.connect(context,new DomainObject(changeOID),RELATIONSHIP_APPLIED_MARKUP,new DomainObject(sMarkupId));
		                    } else if (BOMMgtUtil.isNotNullAndNotEmpty(changeOID)){
					...
					if(ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)) {
						...
						if((DomainConstants.TYPE_ECO.equalsIgnoreCase(strChangeType))|| (ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)))
	                        {
	                        DomainRelationship.connect(context,new DomainObject(changeOID),RELATIONSHIP_APPLIED_MARKUP,new DomainObject(sMarkupId));
	                    } else if (BOMMgtUtil.isNotNullAndNotEmpty(changeOID)){
	                        DomainRelationship.connect(context,new DomainObject(changeOID),RELATIONSHIP_PROPOSED_MARKUP,new DomainObject(sMarkupId));
    ...
    public void generateBOMMarkupXML(Context context,String[] args) throws Exception
    {
       ...
       //Start : IR-054311V6R2011x
        ...
        //Added for TreeOrder Add Next operation
        com.matrixone.jdom.Element addNextEelement = null;
        com.matrixone.jdom.Element selectedElement = null;
        com.matrixone.jdom.Element nextElement = null;
        ...
        while (itr.hasNext())
                {
                    ...
                    else
                    {
						...
						String sOperation = sRelId;
						if(sRelId == null || "AddNext".equalsIgnoreCase(sRelId))
							sRelId = "new";
                        ...
                        sbmListcount++;
                         ...
                         Iterator columnIterator=columnelements.iterator();
                          ...
                          //Added for TreeOrder Add Next operation--Start
                          if( "AddNext".equalsIgnoreCase(sOperation)){
	                          addNextEelement = new com.matrixone.jdom.Element("addNextElement");
	                          relationshipElement.addContent(addNextEelement);
	                          
	                          selectedElement = new com.matrixone.jdom.Element("selectedElement");
	                          selectedElement.setText(eleChild.getAttributeValue("param2"));
	                          addNextEelement.addContent(selectedElement);
	                          
	                          nextElement = new com.matrixone.jdom.Element("nextElemet");
	                          nextElement.setText(eleChild.getAttributeValue("param3"));
	                          addNextEelement.addContent(nextElement);
                          }
                        //Added for TreeOrder Add Next operation--End
 ...
 //If selected part is Alternate
  ...
  public String generatedItemMarkupView(Context context, String args[]) throws Exception
    {
	  ...
	  // 071743 ends here
		...
		StringList dataSL = FrameworkUtil.split(fileContent,"#");
		//Item Markup correction for upgraded stacks - Start
		if(!ReleasePhaseManager.isItemMarkupForSetToProduction(context, objectId) && !dataSL.isEmpty()) {
	        //Lead Time to Lead Time Duration data conversion start
			if(!dataSL.contains(ATTRIBUTE_LEAD_TIME_DURATION)) {
				int leadTimeIndex = dataSL.indexOf(ATTRIBUTE_LEAD_TIME);
				if(leadTimeIndex>-1){
					String oldLTVal = (String) dataSL.get(leadTimeIndex+1);
					String newLTVal = (String) dataSL.get(leadTimeIndex+2);
					oldLTVal = convertLeadTimetoLeadTimeDuration(context, oldLTVal).replace(' ', '_');
					newLTVal = convertLeadTimetoLeadTimeDuration(context, newLTVal).replace(' ', '_');
					int insertIndex=fileContent.lastIndexOf('#');
					fileContent = fileContent.substring(0, insertIndex) + "#"+ATTRIBUTE_LEAD_TIME_DURATION+"#"+oldLTVal+"#"+newLTVal+fileContent.substring(insertIndex, fileContent.length());
				}
			}
	        //Lead Time to Lead Time Duration data conversion end

			//Target Cost Attribute value correction - Start
			int targetCostIndex = dataSL.indexOf(ATTRIBUTE_TARGET_COST);
			if(targetCostIndex>-1){
				String oldTargetCostVal = (String) dataSL.get(targetCostIndex+1);
				String newTargetCostVal = (String) dataSL.get(targetCostIndex+2);
				if(!oldTargetCostVal.contains("_") || !newTargetCostVal.contains("_")) {
					oldTargetCostVal = convertTargetCostToDimension(context, oldTargetCostVal);
					newTargetCostVal = convertTargetCostToDimension(context, newTargetCostVal);

					fileContent = fileContent.replaceAll(ATTRIBUTE_TARGET_COST+"#.*\\d+#.*\\d+#", ATTRIBUTE_TARGET_COST+"#"+oldTargetCostVal+"#"+newTargetCostVal+"#");
        		}
			}
			//Target Cost Attribute value correction - End
		}
		//Item Markup correction for upgraded stacks - End
        ...
        if("create".equals(sAction))
        {
            		...
            		|| EngineeringConstants.ATTRIBUTE_CHANGE_CONTROLLED.equals(attrName))
            {
                  ...
                  String changeControlledValue = JPO.invoke(context, "enoPartManagement", null, "getReviseChangeControlled", JPO.packArgs(newMap), String.class);
            	if(EngineeringConstants.ATTRIBUTE_CHANGE_CONTROLLED.equals(attrName)){
            		sReturn=JPO.invoke(context, "enoECMChangeUX", null, "getHTMLForChangeControlValueOnForm", JPO.packArgs(programMap), String.class);
        			return sReturn;
            	}else{
  ...
  public void createItemMarkup(Context context, String args[]) throws Exception
				...
				if("TRUE".equalsIgnoreCase(changeConnected)) {						//
					sChangeOID = (String)changeActionMap.get(ChangeConstants.RELATED_CA_ID);
				      ...
				      if(dChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST)) {
				    	  EngineeringUtil.createConnectionsWithCAandCR(context, sChangeOID, strCAId, new StringList(objectId));
		            	} 
				      sChangeOID = strCAId;
				}	
    ...
    public void createPlantBOMMarkup(Context context,String args[])
        ...
        // IR-964488: When Plant ID is null, extract plant ID from the given changeOID(ECR)
        if((selectedPlantIds == null || "null".equalsIgnoreCase(selectedPlantIds)) && (changeOID != null))
        {
			String mqlQuery = "print bus $1 select $2 dump";
			String strPlantQuery = EngineeringConstants.SELECT_TO_LEFTBRACE + EngineeringConstants.RELATIONSHIP_MANUFACTURING_RESPONSIBILITY +"].from.id";
			String strPlantID = MqlUtil.mqlCommand(context, mqlQuery, changeOID, strPlantQuery);
			selectedPlantIds = strPlantID;
			System.out.println("plant ID is null in createPlantBOMMarkup: selectedPlantIds -"+ selectedPlantIds);
        }
        ...
        /* For Development Parts */
            ...
            /* Check Control Coming From Save As Command */
                ...
                String vaultName= Person.getPerson(context).getVaultName(context);
					...
					if("true".equals(hasPlantChanges)) {
						DomainObject domMarkup= new DomainObject();
						domMarkup.createObject(context,TYPE_PLANT_BOM_MARKUP,sName,sRevision,POLICY_DEVELOPMENTPARTMARKUP,context.getVault().getName());
                ...
                generatePlantBOMMarkupXML(context,str);
					...
					if("true".equals(hasBOMChanges)) {
						DomainObject domMarkup= new DomainObject();
						domMarkup.createObject(context,TYPE_BOMMARKUP,sName,sRevision,POLICY_DEVELOPMENTPARTMARKUP,context.getVault().getName());
   ...
   public HashMap sendPIMXML(Context context, String args[]) throws Exception
   {
     ...
     */
          ...
          throws Exception
     {
		...
		// For Function FUN073757 & FUN074207
		
		IPartService iPartService = IPartService.getService(context, sPartObjId);
		return Boolean.valueOf(iPartService.isChangeControlled(context));
...
/**
    ...
    **/
	...
	public void saveMarkup(Context context, String args[]) throws Exception {
		...
		if(null!=strObjectIds) {
			...
			strObjectIds=(String) requestMap.get("objectIds")!=null?(String) requestMap.get("objectIds"):(String) requestMap.get("partWhereusedMarkupIds");
    ...
    public int isPromotedFromMarkupActions(Context context, String[] args) throws Exception {
   		try{
			...
			String rejectMarkupOnChangeCancel = context.getCustomData("rejectBGTPMarkupOnChangeCancel");
			if("TRUE".equals(fromMarkupActions)){
				return 0;
			}else if("TRUE".equalsIgnoreCase(rejectMarkupOnChangeCancel)){
				return 0;
			}
			else{
    ...
    public MapList getAplliedMarkupXMLContent(Context context,String[] args) throws Exception
    {
        ...
        java.io.File fMarkupXML = new java.io.File(fEmatrixWebRoot, strXMLFileName);
        try{
            Scanner scanner = new Scanner(fMarkupXML);
			String text = scanner.useDelimiter("\\A").next();
			String docType="<!DOCTYPE ematrix>";
			if(text.contains(docType)) {
				FileWriter fw = new FileWriter(fMarkupXML);
			    fw.write(text.replaceAll(docType,  "<!--"+docType+"-->"));
			    fw.close();
			}
			scanner.close();
           }
	...
	public boolean isMarkupCreateAllowed(Context context,
		 ...
		 BusinessObject newRevBO = dObj.getNextRevision(context);
		...
		if(strCurrent.equalsIgnoreCase(STATE_REVIEW)||strCurrent.equalsIgnoreCase(STATE_APPROVED)){
			return false;
		}else if((STATE_PART_RELEASE.equals(strCurrent) && newRevBO.exists(context)) && !"true".equalsIgnoreCase(getWhereUsed)){
			return false;
   ...
   //For ECM Convergence Start 
     ...
     **/
  	...
  	/**
     * Checks for any Approved Markups for Parts.
     *
     * @param context the eMatrix <code>Context</code> object
     * @param args    holds the following input arguments:
     *             0 - ObjectId
     ...
     * @since         EngineeringCentral X3
     **/
    public static int checkAppliedMarkupsInFrozen(Context context,String[] args) throws Exception
    {
    	//Multitenant
    	int val = checkAppliedMarkUps(context, args);
    	if(val==1){ return val;}
        
    	String strPartId = args[0];
        DomainObject domNewRevPart = new DomainObject(strPartId);
        BusinessObject busObj = domNewRevPart.getPreviousRevision(context);
        if(busObj.exists(context)){
        	String strWhereClause = "(current=="+ proposedState+ " || current=="+ approvedState + ") && (Policy != '"+POLICY_DEVELOPMENTPARTMARKUP + "')" ;
        	DomainObject domObj = DomainObject.newInstance(context, busObj);
        	if(EngineeringConstants.STATE_EC_PART_RELEASE.equals(domObj.getInfo(context, SELECT_CURRENT))){
        		MapList mapListMarkups = domObj.getRelatedObjects(context,RELATIONSHIP_EBOM_MARKUP,"*",new StringList(DomainConstants.SELECT_ID),new StringList(),false,true,(short) 1,strWhereClause,null,0);
        		boolean approvedMarkUpsExist = false;
        		for(int i=0;i<mapListMarkups.size();i++){
    	        	String objId =(String) ((Map) mapListMarkups.get(i)).get(DomainConstants.SELECT_ID);
    	        	if(UIUtil.isNotNullAndNotEmpty(objId) && !ReleasePhase.isItemMarkupForSetToProduction(context, objId)){
    	        		approvedMarkUpsExist = true;
    	        		break;
    	        	}
    	        }
        		if (approvedMarkUpsExist)
	            {
	        		String sApproveNewErr = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.MarkupActions.DevPartsApplyError");
	            	emxContextUtil_mxJPO.mqlNotice(context,sApproveNewErr);
	                return 1;
	            }
        	}
        }
  	...
  	/**
     * Checks for any Applied Markups for Part promotion From Inwork to Frozen
     *
     * @param context the eMatrix <code>Context</code> object
     * @param args    holds the following input arguments:
     *             0 - ObjectId
     * @return        a eMatrix <code>MapList</code> object having the list of all Objects which are required checkbox to be displayed in the Table List Page.
     * @throws        f the operation fails
     * @since         EngineeringCentral R419.FD07
     **/
    // Promotes from Inwork to Approved only if all the markups are either Rejected or Applied checkApprovedMarkUps
    public static int checkAppliedMarkUps(Context context,String[] args) throws Exception
    {
    	String sApproveErr=EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.MarkupActions.DevPartsApplyError");
        ...
        DomainObject doPart = new DomainObject(strPartId);
    	 ...
    	 String strWhereClause = "current=="+ proposedState + " || current=="+ approvedState;

        String strTypePattern = TYPE_ITEMMARKUP + "," + TYPE_EBOMMARKUP+ "," + TYPE_BOMMARKUP;
        MapList mapListMarkups = doPart.getRelatedObjects(context,RELATIONSHIP_EBOM_MARKUP,strTypePattern,new StringList(DomainConstants.SELECT_ID),new StringList(),false,true,(short) 1,strWhereClause,null,0);
    ...
    /**
     * Validates the connected MArkUps before Revision
     * @param context
     * @param args
     * @return
     * @throws Exception
     */
    public int validateMarkUpBeforeApplyOnRevise(Context context,String[] args) throws Exception
    {
    	try{
	    	checkApprovedMarkupsOnPartForRevise(context, args);//
	    	
	    	checkforMarkUpsConnectedToCancelledCAForRevise(context, args);//
	    	
	    	checkConflictMarkupsonRevise(context, args);//
    ...
    /**
     * Revise Check trigger on type Part 
     * If any proposed Markups connected to part then dont allow revising of the Part
     * @param context the eMatrix <code>Context</code> object
     * @param  args[]  holds the Part Id.
     * @return        a boolean value as true if no approved MarkUp connected to Part. 
     * @throws        Exception if the operation fails
     * @since         EngineeringCentral R419.FD07
     **/
  	public int checkforMarkUpsConnectedToCancelledCAForRevise(Context context, String args[])throws Exception
  	{
  		//Multitenant
          ...
          StringList strlObjectSelects = new StringList(1);
          strlObjectSelects.addElement(DomainConstants.SELECT_ID);
          strlObjectSelects.addElement(DomainConstants.SELECT_NAME);
          strlObjectSelects.addElement(DomainConstants.SELECT_POLICY);

          String strWhereClause = "(current!="+ rejectedState + ")";
          String strTypePattern = TYPE_ITEMMARKUP + "," + TYPE_EBOMMARKUP+ "," + TYPE_BOMMARKUP;
          MapList mapListMarkups = partObj.getRelatedObjects(context,RELATIONSHIP_EBOM_MARKUP,strTypePattern,strlObjectSelects,new StringList(),false,true,(short) 1,strWhereClause,null,0);

          StringList markUpList = PartMgtUtil.getSListForTheKey(mapListMarkups, DomainConstants.SELECT_ID);
          try{
	      return isMarkupConnectedToCancelledChnage(context, markUpList);
          }catch(Exception e){
     ...
     * @since         EngineeringCentral R419.FD07
     **/
  	public int checkApprovedMarkupsOnPartForRevise(Context context, String args[])throws Exception
  	{
  		//Multitenant
          ...
          String strWhereClause = "(current =="+ proposedState + ") && (Policy!='" + POLICY_DEVELOPMENTPARTMARKUP+"')";
          String strTypePattern = TYPE_ITEMMARKUP + "," + TYPE_EBOMMARKUP+ "," + TYPE_BOMMARKUP;
          MapList mapListMarkups = partObj.getRelatedObjects(context,RELATIONSHIP_EBOM_MARKUP,strTypePattern,strlObjectSelects,new StringList(),false,true,(short) 1,strWhereClause,null,0);

          if (mapListMarkups.size() > 0)
          {
        	  throw new Exception(sApproveErr);
          }
    ...
    /**
     * Checkes if MarkUp is connected to cancelled CA. 
     * @param context
     * @param sMarkupObjId
     * @return returns 1 if part is connected to cancelled CA.
     * @throws Exceptionf the operation fails
     * @since         EngineeringCentral R419.FD07
     *
     **/
	public int isMarkupConnectedToCancelledChnage(Context context,StringList sMarkupObjIdList) throws Exception{
		String strMessage = "";
		for(int i=0;i<sMarkupObjIdList.size();i++){
			String markupObjId = (String)sMarkupObjIdList.get(i);
			DomainObject markUpObj = new DomainObject(markupObjId);
			StringList objSelList = new StringList();
			objSelList.add(DomainConstants.SELECT_NAME);
			objSelList.add(DomainConstants.SELECT_TYPE);
			objSelList.add("to["+RELATIONSHIP_APPLIED_MARKUP+"].from.current");
			objSelList.add("to["+RELATIONSHIP_EBOMMARKUP+"].from.name");
			
			Map markupMapList = markUpObj.getInfo(context, objSelList);
			if(TYPE_BOMMARKUP.equals(markupMapList.get(DomainConstants.SELECT_TYPE))|| TYPE_EBOMMARKUP.equals(markupMapList.get(DomainConstants.SELECT_TYPE)) || TYPE_ITEMMARKUP.equals(markupMapList.get(DomainConstants.SELECT_TYPE))){
	    		String currentCAState = markUpObj.getInfo(context, "to["+RELATIONSHIP_APPLIED_MARKUP+"].from.current");
	    		if(ChangeConstants.STATE_CHANGE_ACTION_CANCEL.equals(currentCAState)){
	    			String[] messageValues = new String[2];
            		messageValues[0] = (String) markupMapList.get(DomainConstants.SELECT_NAME);
            		messageValues[1] = (String) markupMapList.get("to["+RELATIONSHIP_EBOMMARKUP+"].from.name");
            		strMessage = strMessage +"\n"+ MessageUtil.getMessage(context,null,"emxEngineeringCentral.Alert.CancelledCAMarkup",messageValues,null,context.getLocale(),"emxEngineeringCentralStringResource");
	    		}
			}
		}
	 ...
	 /**
	  * Checks for conflicts between BOM markups that will be applied during Part revision.
	  * @param context
	  * @param args
	  * @return returns 1 if Conflict exists among the BOM Markups connected to part 
	  * @throws Exceptionf the operation fails
     * @since         EngineeringCentral R419.FD07
     **/
    //Shd change the name of the method megreApprovedMarkupsOnChange
    public int checkConflictMarkupsonRevise(Context context, String args[]) throws Exception
   {   
       String strPartId = args[0];
       String strTypePattern = TYPE_BOMMARKUP + ","+ TYPE_EBOMMARKUP;

       DomainObject domObj = new DomainObject(strPartId);
       StringList strlObjectSelects = new StringList(1);
       strlObjectSelects.addElement(DomainConstants.SELECT_ID);
       strlObjectSelects.addElement(DomainConstants.SELECT_TYPE);
       strlObjectSelects.addElement(DomainConstants.SELECT_CURRENT);
       strlObjectSelects.addElement(DomainConstants.SELECT_POLICY);

       StringList markupIDList = new StringList();
       
       String whereExpression = "(current==" + approvedState + ") || (Policy == '"+POLICY_DEVELOPMENTPARTMARKUP+ "' && current==" + proposedState + ")"; 
       MapList markupMapList = domObj.getRelatedObjects(context,RELATIONSHIP_EBOMMARKUP,strTypePattern,strlObjectSelects,null,false,true,(short)1,whereExpression,null,0);
       
       markupIDList = PartMgtUtil.getSListForTheKey(markupMapList, DomainConstants.SELECT_ID); 
   	
       if(markupIDList.size() > 0) {
    	   String stre="true";
    	   
    	   HashMap finalMap=new HashMap();
           finalMap.put("chhId",new StringList(strPartId));
    	   
           HashMap programMapBM = new HashMap();
   		   programMapBM.put("markupIds", markupIDList);
   		   
   		   stre = (String)checkBOMConflict(context,JPO.packArgs(programMapBM));
           
   		   if("false".equalsIgnoreCase(stre))
           {
        	   String sConflictErr =EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.MarkupActions.ConflictError");
        	   throw new Exception(sConflictErr);
           }
       }
       return 0;
   }
    /**
     * This method is for collaborating Part modified using ItemMarkup. 
     * @param context
     * @param objectIdList
     * @throws Exceptionf the operation fails
     * @since         EngineeringCentral R419.FD07
     **/
	public void collaboratePartAfterItemMarkApply(Context context,StringList objectIdList) throws Exception{
		if(objectIdList.size()>0){
				IUserFactChangeLogService iUserFactChangeLogService = IUserFactChangeLogService.getService();
				iUserFactChangeLogService.updateObjectUserFact(context, objectIdList, IUserFactChangeLogService.USER_EVENT_OBJECT_MODIFY);
				
				ArrayList<String> list = new ArrayList<String>();
				list.addAll(objectIdList);
				IPartCollaborationService iPartCollabService = new PartCollaborationService();
	...
	/**
	 * Applies Approved Markups On Part revision.
	 * @param context
	 * @param args
	 * @return
	 * @throws Exceptionf the operation fails
     * @since         EngineeringCentral R419.FD07
     **/
  	public int applyMarkupsOnRevise(Context context, String args[]) throws Exception
    {
        ...
        IPartService iPartService = IPartService.getService(context, sPartId);
        StringList iMKpartIdList = new StringList();
        if(iPartService.getInfo(context).isReleased()){
	        ...
	        String whereExpression = "(current==" + approvedState + ") || (Policy == '"+POLICY_DEVELOPMENTPARTMARKUP+ "' && current==" + proposedState + ")"; 
	        MapList markupMap = domobj.getRelatedObjects(context,RELATIONSHIP_EBOMMARKUP,strTypes,selectStmts,null,false,true,(short)1,whereExpression,null,0);
	        
	        	if(markupMap.size()>0){
	        		DomainObject doPart = new DomainObject(sPartId);
					String strRevPartId =doPart.getInfo(context, "next.id");
					if(UIUtil.isNullOrEmpty(strRevPartId)){ return val;}
		        	changeActionId = ECMUtil.getWorkUnderChange(context);
		        	changeActionId = (UIUtil.isNotNullAndNotEmpty(changeActionId))?changeActionId:"";
					ContextUtil.startTransaction(context, true);
					ECMUtil.setWorkUnderChange(context, changeActionId);
				    try{
							Iterator itrMarkups = markupMap.iterator();
			                 while(itrMarkups.hasNext())
			                 {
								Map mapMkp = (Map)itrMarkups.next();
								String strMarkupObjId = (String)mapMkp.get(DomainConstants.SELECT_ID);
								String strMarkupType = (String)mapMkp.get(DomainConstants.SELECT_TYPE);
									DomainObject doMarkup = new DomainObject(strMarkupObjId);

					                if(TYPE_BOMMARKUP.equalsIgnoreCase(strMarkupType) || TYPE_EBOMMARKUP.equalsIgnoreCase(strMarkupType))
					                {
					                    val = applyBOMMarkup(context,strMarkupObjId,strRevPartId,changeActionId);
					                }
					                else if(TYPE_ITEMMARKUP.equalsIgnoreCase(strMarkupType))
					                {	
					                    applyingItemMarkup(context,strMarkupObjId,strRevPartId,changeActionId);
					                    iMKpartIdList.add(strRevPartId);
					                }if (!TYPE_EBOMMARKUP.equalsIgnoreCase(strMarkupType))
					                {
				                        doMarkup.setAttributeValue(context,ATTRIBUTE_BRANCH_TO,"None");
									}
	...
	/**
	  * Converts Lead Time Attribute Data to corresponding Lead Time Duration Data (with unit)
	  * @param context Context object
	  * @param leadTime value which needs to be converted.
	  * @return returs the corresponding Lead Time Duration Data (with unit) 
	  * @throws Exceptionf the operation fails
    **/
  	
  	private String convertLeadTimetoLeadTimeDuration(Context context, String leadTime) throws Exception {
  		String leadTimeDuration="";
  		if("Unassigned".equals(leadTime)) {
  			AttributeType attTypeLTD = new AttributeType(ATTRIBUTE_LEAD_TIME_DURATION);
        	attTypeLTD.open(context);
        	leadTimeDuration=attTypeLTD.getDefaultValue()+" Day";
        	attTypeLTD.close(context);
  		} else {
  			leadTimeDuration = leadTime.split(" ")[0]+" Week";
  		}
  		return leadTimeDuration;
  		
  	}
  	
  	private String convertTargetCostToDimension(Context context, String targetCost) throws Exception {
  		if(!targetCost.contains("_")) {
  			AttributeType attTypeTargetCost = new AttributeType(ATTRIBUTE_TARGET_COST);
  			attTypeTargetCost.open(context);
        	Dimension dimObj=attTypeTargetCost.getDimension(context);
        	String defaultUnit = dimObj.getDefaultUnit(context).getLabel();
        	attTypeTargetCost.close(context);
  			String targetCostWithDimension = targetCost + "_" + defaultUnit;
  			return targetCostWithDimension;
      	}
  		return targetCost;
 	}
	/**
  	 * This method rejects the Set To Production Item Markup when corresponding Change Order is cancelled.  
  	 * @param context
  	 * @param args
  	 * @return
  	 * @throws Exception
  	 * Trigger: TypeChangeActionChangePolicyAction
*/
	@com.matrixone.apps.framework.ui.ProgramCallable
	public int rejectItemMarkup(Context context, String[] args) throws Exception{
		final String RELATIONSHIP_APPLIED_MARKUP =PropertyUtil.getSchemaProperty(context, "relationship_AppliedMarkup");
		final String SET_TO_PRODUCTION = "Set To Production";
  		final String STR_INTERFACE = "interface";

  		String sChangeAction = args[0];
  		String sNewPolicy = args[1];
  		StringList sItemMarkupInterface = new StringList();
  		StringList selectStmnt = new StringList();
  		String selectMarkupInterface = "";
  		DomainObject domCA = DomainObject.newInstance(context, sChangeAction);

  		try{

  			if(ChangeConstants.STATE_CHANGE_ACTION_CANCEL.equalsIgnoreCase(sNewPolicy)){
  				String strRelPattern = RELATIONSHIP_APPLIED_MARKUP;
  				String strTypePattern = TYPE_ITEMMARKUP;

  				StringList strlObjectSelects = new StringList(1);
  				strlObjectSelects.addElement(DomainConstants.SELECT_ID);
  				strlObjectSelects.addElement(DomainConstants.SELECT_NAME);
  				strlObjectSelects.addElement(STR_INTERFACE);

  				MapList  mapListMarkups = domCA.getRelatedObjects(context,
  						strRelPattern, // relationship pattern
  						strTypePattern, // object pattern
  						strlObjectSelects, // object selects
  						new StringList(1), // relationship selects
  						false, // to direction
  						true, // from direction
  						(short) 1, // recursion level
  						null, // object where clause
  				...
  				Iterator markupItr = mapListMarkups.iterator();
  				Map tempMap = new HashMap();
  				while(markupItr.hasNext()){
  					tempMap = (Map)markupItr.next();
  					selectMarkupInterface = (String)tempMap.get("interface");
  					if(BOMMgtUtil.isNotNullAndNotEmpty(selectMarkupInterface) && selectMarkupInterface.equalsIgnoreCase(SET_TO_PRODUCTION)){
  						context.setCustomData("rejectBGTPMarkupOnChangeCancel", "TRUE");
  						DomainObject domItemMarkup = DomainObject.newInstance(context, (String)tempMap.get("id"));
  						domItemMarkup.promote(context);
  						domItemMarkup.promote(context);
  						context.removeFromCustomData("rejectBGTPMarkupOnChangeCancel");
  					}
  				}
  			}
  		}catch(Exception ex){
