                        ...
                        if(locale != null){
                            ...
                            if(isRangeAttribute) {
                            	rangeValues.forEach(value -> {
                            		String nlsKey = (String) value;
                            		if(attribute.get("type").equals("real") && value.toString().endsWith(".0")) {
                            			nlsKey = value.toString().replace(".0","");
                            		}
                                	String rangeProperty = "emxFramework.Range." + attributeName.replaceAll(" ", "_") + "." + nlsKey;
                                    String rangeValueFromPropertiesFile = EnoviaResourceBundle.getProperty(context,
                                            "emxFrameworkStringResource", locale, rangeProperty);
                                    if(rangeValueFromPropertiesFile.contains("emxFramework.Range.")){
                                    	rangeValueFromPropertiesFile = (String) value;
                                    }
                                    else {
