...
public class IEFUtil_mxJPO
{
    ...
    public  MapList getObjectMapList(Context context, Map HRefParams, String[] args) throws MatrixException
    {
	   ...
	   String isNavigationInSaveNewDocument= null;
       if(args.length >4)
   		isNavigationInSaveNewDocument= args[4];
		...
		/*IR-844372 : navigation performance issue
		 * Ideally relationship_ProjectVaults,relationship_VaultedDocuments,relationship_VaultedDocumentsRev2,relationship_SubVaults, relationship_LinkedFolders relationships used to get content in bookmark root/ bookmark
		 * for save as case, we will only sent folder info to client, so will remove below relationships
		 * 1. relationship_VaultedDocumentsRev2
		 * 2.relationship_VaultedDocuments
		 * 3. relationship_ReferenceDocument
		 * 4. relationship_ClassifiedItem
		 * */
		if(isNavigationInSaveNewDocument!= null && isNavigationInSaveNewDocument.equalsIgnoreCase("true")){
				   // StringList excludeRelationshipList = new StringList();
		ArrayList excludeRelationshipList = new ArrayList();
		excludeRelationshipList.add("relationship_VaultedDocumentsRev2");	//relationship bet bookmark and Document
		excludeRelationshipList.add("relationship_VaultedDocuments");		//relationship bet bookmark and Document
		excludeRelationshipList.add("relationship_ReferenceDocument");		//relationship bet general class and Document
		excludeRelationshipList.add("relationship_ClassifiedItem");		//relationship bet general class and Document

		StringBuffer strBuffer = new StringBuffer();
		String[] parentRelNameArr = parentRelName.split(",");
		for (int i = 0; i < parentRelNameArr.length; i++) {
			String val = parentRelNameArr[i];
			if(!excludeRelationshipList.contains(val)){
		...
		parentRelName = strBuffer.toString();
		}

		// End of navigation performance issue
