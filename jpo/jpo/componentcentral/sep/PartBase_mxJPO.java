    ...
    public StringList getMEPLocationNamesHTMLOutput(Context context,
        ...
        try {
            ...
            if (objectList != null && objectList.size() > 0) {
                ...
                StringList selectsMultivalList = new StringList();
                ...
                selectsMultivalList.add(manEquAloResNameSel);
                selects.add(manEquAloResNameSel);

                //DomainObject.MULTI_VALUE_LIST.add(manEquAloResIdSel);
                selectsMultivalList.add(manEquAloResIdSel);
                ...
                selectsMultivalList.add(aloResNameSel);
                selects.add(aloResNameSel);

                //DomainObject.MULTI_VALUE_LIST.add(aloResIdSel);
                selectsMultivalList.add(aloResIdSel);
                selects.add(aloResIdSel);

                // Get Location Name and Id information
                MapList locMaplist = getInfo(context, oidList, selects, selectsMultivalList);
    ...
    public StringList getMEPLocationStatusHTMLOutput(Context context,
        ...
        try {
            ...
            if (objectList != null && objectList.size() > 0) {
                ...
                selectsMultiValList.add(manEquAloResStaSel);
                selects.add(manEquAloResStaSel);

                //DomainObject.MULTI_VALUE_LIST.add(aloResStaSel);
                selectsMultiValList.add(aloResStaSel);
                selects.add(aloResStaSel);

                // Get Location Status Info
                MapList statusMaplist = getInfo(context, oidList, selects, selectsMultiValList);
    ...
    public StringList getMEPLocationPreferenceHTMLOutput(Context context,
        ...
        try {
            ...
            if (objectList != null && objectList.size() > 0) {
                ...
                selectsMultivalList.add(manEquAloResPreSel);
                selects.add(manEquAloResPreSel);

                //DomainObject.MULTI_VALUE_LIST.add(aloResPreSel);
                selectsMultivalList.add(aloResPreSel);
                selects.add(aloResPreSel);

                // Get Location Preference information
                MapList prefMaplist = getInfo(context, oidList, selects, selectsMultivalList);
...
@com.matrixone.apps.framework.ui.PostProcessCallable
    ...
    public StringList getMEPEnterpriseParts(Context context, String[] args)
        ...
        StringList selectMultivalList = new StringList(8);
        selectMultivalList.add(LocContextEntId);
        selectMultivalList.add(LocContextEntName);
        selectMultivalList.add(LocContextEntRev);
        selectMultivalList.add(CorpContextEntId);
        selectMultivalList.add(CorpContextEntType);
        selectMultivalList.add(CorpContextEntName);
        selectMultivalList.add(CorpContextEntRev);
        ...
        try {
            ...
            MapList listEquiv = DomainObject.getInfo(context, arrobjectId,
                    selectStmts, selectMultivalList);
...
/**
    ...
    public MapList getMEPSearchResult(Context context, String[] args)
        ...
        try {
            ...
            organizationList = DomainObject.findObjects(context, sType, sName,
                    sRev, sOwner, null, sbWhereExp.toString(), null, true,
    ...
    public void addECParttoSEP(Context context,String ecPartIds[]) throws FrameworkException{
        try{
			...
			// changes done by ssi17 for  IR-567064-3DEXPERIENCER2019x
            String busName =(String)requestMap.get("Name");
            DomainObject ecPartObj = new DomainObject(ecPartId);
            ecPartObj.setAttributeValue(context, CPCConstants.ATTRIBUTE_V_NAME, busName);
            
            if(policy.equals(DomainConstants.POLICY_MANUFACTURER_EQUIVALENT))
            	DomainRelationship.connect(context,ecPartObj,DomainConstants.RELATIONSHIP_MANUFACTURER_EQUIVALENT,new DomainObject(sepPartId));
			else
			    DomainRelationship.connect(context,ecPartObj,CPCConstants.RELATIONSHIP_SUPPLIER_EQUIVALENT,new DomainObject(sepPartId));
         // end of ssi17
            ContextUtil.commitTransaction(context);
        }catch(Exception err){
            ContextUtil.abortTransaction(context);
   ...
   public StringList checkAVCLicense(Context context, String args[])  throws FrameworkException {
	   ...
	   try{
		   ...
		   for(int itr = 0; itr < mlChecklists.size(); itr++){
			   ...
			   slIncludeIds.add((String)map.get(DomainConstants.SELECT_ID));
			}
...
@com.matrixone.apps.framework.ui.PostProcessCallable
  	  ...
  	  */
         ...
         */
                ...
                throws Exception
         {
       	  ...
       	  //changes done by ssi17 for  IR-542989-3DEXPERIENCER2019x
		  if (partObj.isKindOf(context,DomainConstants.TYPE_PART) &&
       		  partPolicy.equals(DomainConstants.POLICY_MANUFACTURER_EQUIVALENT))
       	  {
       		  hasAccess = new Boolean(true);
       	  }
	...
	//added function for  IR-538028-3DEXPERIENCER2018x
	/**
	 * Method to display the Policy field in the create part webform 
	 * 
	 * @param context
	 *            the eMatrix <code>Context</code> object.
	 * @param args
	 ...
	 * @since V6R2017-12
	 */
	public String showPartPolicy(Context context, String[] args) throws Exception {
		StringBuffer resultBuffer = new StringBuffer();
		String typePart = PropertyUtil.getSchemaProperty(context, "type_Part");
		StringList allPolicyList = CPCPart.getPartPolicies(context, typePart);
		String policyName = "";
		StringList policyList = new StringList();
		for (Object policyObj : allPolicyList) {
			policyName = (String) policyObj;
			if (policyName.equals(CPCConstants.POLICY_DEVELOPMENT_PART)
					|| policyName.equals(CPCConstants.POLICY_EC_PART)) {
