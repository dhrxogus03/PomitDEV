...
public class emxDrawingMarkupBase_mxJPO extends emxDomainObject_mxJPO
{
  ...
  public Map createDrawingMarkupJPO(Context context, String[] args) throws Exception {
		...
		connectDrawingMarkup(context,(String) programMap.get("objectId"), markupId);
		return mapReturn;
	}
	public void connectDrawingMarkup(Context context, String fromObjectId, String toObjectId) throws Exception{
	try {
			if(UIUtil.isNotNullAndNotEmpty(fromObjectId) && UIUtil.isNotNullAndNotEmpty(toObjectId)){
				DomainObject fromBusObj = new DomainObject(fromObjectId);
				DomainObject toBusObj = new DomainObject(toObjectId);
				ContextUtil.pushContext(context);
				DomainRelationship.connect(context, fromBusObj,
						RELATIONSHIP_MARKUP, toBusObj);
