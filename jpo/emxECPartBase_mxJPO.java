...
import com.dassault_systemes.enovia.partmanagement.modeler.interfaces.validator.IPartLifecycleValidator;
...
// Added for IR-021267V6R2011
   public static final String  RELATIONSHIP_EBOM_SUBSTITUTE =   PropertyUtil.getSchemaProperty("relationship_EBOMSubstitute");
	...
	public static final String SELECT_PRODUCT_ID = "from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to["+TYPE_PLMENTITY+"]."+DomainConstants.SELECT_ID;
	public static final String SELECT_PRODUCT_NAME = "from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to["+TYPE_PLMENTITY+"]."+DomainConstants.SELECT_NAME;
	public static final String SELECT_PRODUCT_ATTR_USAGE = "from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to["+TYPE_PLMENTITY+"].attribute["+TYPE_PLMENTITY+".V_usage]";  
...
//Added for IR-021267
                    ...
                    throws Exception
    {
        ...
        try
        {
            ...
            /*
             * commented to address  IR-510823
            String select = null;
            ...
            */
            ...
            //Fix for IR-510823 : Start
            DomainObject doNextRev = new DomainObject(this.getNextRevision(context));
            
            // If there is no relationship name passed in then default to EBOM
            String strRelationship = DomainObject.RELATIONSHIP_EBOM;
            if ("relationship_ManufacturerEquivalent".equals(rel_SymbolicName))
            {
            	strRelationship = DomainObject.RELATIONSHIP_MANUFACTURER_EQUIVALENT;
            }
            
            StringList slRelSelect = new StringList ();
            slRelSelect.add(DomainObject.SELECT_RELATIONSHIP_ID);            
            MapList mlResults = doNextRev.getRelatedObjects(context,
            ...
            Map mapResult = null;
            if (itr.hasNext()) {
            	mapResult = (Map)itr.next();
            	relId = (String) mapResult.get(DomainObject.SELECT_RELATIONSHIP_ID);
                // Set start and end effectivity to blank for this new revision of
                // assembly part
                DomainRelationship.setAttributeValues(context, relId, attributes);
            }
...
//Modified for Bug # 359414 - ends
	        boolean isExp = isScientificNotation(quantity);
	        if(isExp) {
	        	java.text.NumberFormat nf = java.text.NumberFormat.getInstance();
				double len = Double.parseDouble(quantity.substring(quantity.indexOf('-')+1))+2;
				nf.setMaximumFractionDigits((int)len);
    ...
    try {
        ...
        if(objList != null && objList.size() > 0)
        {
           ...
           //069732
            ...
            SelectList multiValueSelects = new SelectList();
            multiValueSelects.add("relationship[" + strRelAffectedItem + "].from.current");
            multiValueSelects.add("relationship["+strRelAffectedItem+"].from.type");
            
            //get ECR/ECO states
            //MapList totalresult = DomainObject.getInfo(context, oidList, objSelects);
            MapList totalresult = DomainObject.getInfo(context, oidList, objSelects, multiValueSelects);
    ...
    if("ALL_VAULTS".equals(txtVaultOption) || "".equals(txtVaultOption))
    {
     ...
     txtVault = txtVault.trim();
       ...
       try
       {
           ...
           mepList =  findObjects(context, txtType, txtName, txtRev, txtOwner, "*",
 ...
 //end of fix
                    ...
                    throws Exception
    {
        try
        {
            ...
            if (isMEP(context))
            {
                ...
                boolean sCompEngrRoleExist= EngineeringUtil.isRoleExist(context, sCompEngRoleName);
                ...
                if (sCompEngrRoleExist && !origPerson.hasRole(context, sCompEngRoleName))
                {
   ...
   */
         ...
         throws Exception
   {
	   ...
	   multiValueSelects.addElement(LocContextEntId);
       multiValueSelects.addElement(LocContextEntType);
       multiValueSelects.addElement(LocContextEntName);
       multiValueSelects.addElement(LocContextEntRev);
       multiValueSelects.addElement(CorpContextEntId);
       multiValueSelects.addElement(CorpContextEntType);
       multiValueSelects.addElement(CorpContextEntName);
       multiValueSelects.addElement(CorpContextEntRev);
       multiValueSelects.addElement(LocContextMEPId);
       multiValueSelects.addElement(LocContextMEPType);
       multiValueSelects.addElement(LocContextMEPName);
       multiValueSelects.addElement(LocContextMEPRev);
       multiValueSelects.addElement(CorpContextMEPId);
       multiValueSelects.addElement(CorpContextMEPType);
       multiValueSelects.addElement(CorpContextMEPName);
       multiValueSelects.addElement(CorpContextMEPRev);
       ...
       try
       {
           ...
           MapList listEquiv = DomainObject.getInfo(context, arrobjectId, selectStmts, multiValueSelects);
                ...
                while (j.hasNext())
                {
                      ...
                      if (partId != null && !("").equals(partId))
                      {
                        ...
                        if (TYPE_MPN.equals(type)) {
							...
							if((DomainConstants.TYPE_COMPANY).equalsIgnoreCase(type)){
								partName = (String) nameMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
							}
                        }
                        //IR-055641 Ends
                        idx++;
         ...
         throws Exception
   {
        ...
        try
        {
            ...
            if(objectList != null && objectList.size() > 0)
            {
                ...
                multiValueSelects.add(manEquAloResNameSel);

                //DomainObject.MULTI_VALUE_LIST.add(manEquAloResIdSel);
                selects.add(manEquAloResIdSel);
                multiValueSelects.add(manEquAloResIdSel);
...
//Added for IR-021267
    ...
    try {
		...
		String[] relSelect = new String[selectRelStmts.size()];
		String[] objSelect =  new String[selectStmts.size()];
		relSelect = (String[]) selectRelStmts.toArray(relSelect);
		objSelect = (String[]) selectStmts.toArray(objSelect);
		iBOMFilterIngress.setExpandLevel(String.valueOf(level));
		iBOMFilterIngress.setObjectSelect(objSelect);
		iBOMFilterIngress.setRelationshipSelect(relSelect);
		iBOMFilterIngress.setObjectWhereExpression(sObjWhereCond.toString());
		
		IBOMService ibomService = IBOMService.getService(context, partId, false);
		if(!"Latest".equals(selectedFilterValue)){
			ebomList = ibomService.getBOM(context, iBOMFilterIngress);
			/*ebomList = partObj.getRelatedObjects(context,
                                         DomainConstants.RELATIONSHIP_EBOM,  // relationship pattern
                                         DomainConstants.TYPE_PART,                  // object pattern
                                         selectStmts,                 // object selects
...
*/		}
        	...
        	iBOMFilterIngress.addObjectSelect("last.id");
			iBOMFilterIngress.setExpandLevel("1");
        	ebomList = ibomService.getBOM(context, iBOMFilterIngress);
        	/*ebomList = partObj.getRelatedObjects(context,
                    DomainConstants.RELATIONSHIP_EBOM,  // relationship pattern
                    DomainConstants.TYPE_PART,                  // object pattern
    ...
    * @author Garima
    ...
    public HashMap getBOMViewFilters(Context context, String args[])
      ...
      slBOMFilterVals.add("Engineering");
	   ...
	   slBOMFilter.setElementAt(slBOMFilter.elementAt(iIndex), 0);

            
            slBOMFilterVals.setElementAt(slBOMFilterVals.firstElement(), iIndex);
          
		   slBOMFilterVals.setElementAt(slBOMFilterVals.elementAt(iIndex), 0);
        }
...
/**
    ...
    public void createMPN(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            boolean sCompEngrRoleExist= EngineeringUtil.isRoleExist(context, sCompEngrRole);
			 boolean sDesignEngrRoleExist= EngineeringUtil.isRoleExist(context, sDesignEngrRole);
			  boolean sMfgEngrRoleExist= EngineeringUtil.isRoleExist(context, sMfgEngrRole);
            if (doEntPart != null) {
                ...
                if ((sCompEngrRoleExist && context.isAssigned(sCompEngrRole)) || (sDesignEngrRoleExist && context.isAssigned(sDesignEngrRole)) || (sMfgEngrRoleExist && context.isAssigned(sMfgEngrRole))) {
    ...
    try{
		...
		selectList.addElement(DomainObject.SELECT_NAME);
        selectList.addElement(DomainObject.SELECT_ID);
		selectList.addElement(DomainConstants.SELECT_ATTRIBUTE_TITLE);
        ...
        String defaultManufacturer = (String) compMap
                .get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
...
* @since Engineering Central - X3
    ...
    throws Exception
    {
		...
		IBOMIngress iBOMIngress;
		try
		{
			
			String workUnderChangeId = PartMgtUtil.getWorkUnderChangeId(context, parentObjectId);
			//iBOMService.setAuthoringChange(workUnderChangeId);
			ECMUtil.setWorkUnderChange(context, workUnderChangeId);
		...
		for (int i = 0; i < chgRowsMapList.size(); i++) {
							...
							iBOMIngress = IBOMIngress.getService();
		...
		if ("add".equals(markup)) {
		                        ...
		                        DomainRelationship domRelation = null;
		                        iBOMIngress.setChildId( childObjectId );
		                        iBOMIngress.setBOMAttributeMap( hmRelAttributesMap );
		                        
		                        if ("replace".equals(strParam3)) {
		                        	
		                        	iBOMIngress.setReplaceByObjectId(childObjectId);
		                        	
		                        	iBOMService = IBOMService.getService(context, strParam1, true);
		                        	iBOMService.setAuthoringChange(workUnderChangeId);
		                        	
		                        	iBOMService.replace(context, iBOMIngress);		                        	
		                        	
		                        	//DomainRelationship.setToObject(context, strParam1, childObj);
		                        	domRelation = new DomainRelationship(strParam1);
		                        } else {
		                        	iBOMService = IBOMService.getService(context, parentObjectId, false);
		                        	iBOMService.setAuthoringChange(workUnderChangeId);
		                        	iBOMService.add( context, iBOMIngress );
		                        	
		                        	StringList slIdList = iBOMService.getInstanceIds();
		                        	if(slIdList != null && slIdList.size()>0){
		                        		domRelation = new DomainRelationship((String)slIdList.get(0));
		                        	}
		                        }
...
//								HashMap items = new HashMap();
								...
								if (!"replace".equals(strParam1)) {
									...
									iBOMService = IBOMService.getService(context, sRelId, true);
									iBOMService.setAuthoringChange(workUnderChangeId);
									
									iBOMService.remove(context, IBOMIngress.getService());
		...
		catch (PartMgtException pmx) {
			doc.put("Action", "ERROR");
			
			if ( "emxBOMPartManagement.BOMOperation.validation.SelectAuthoringChange".equals( pmx.getPropertyKey() ) ) {
				
				DomainObject domObject = DomainObject.newInstance(context, parentObjectId);
				
				String errorMessage = PartMgtUtil.getDisplayPropertyValue(context, "emxPartManagement.BOMCompare.Validation.ChangeControlled", PartMgtConstants.PART_MGT_RESOURCE_FILE);
				
				errorMessage = errorMessage.replace( "$1", domObject.getName(context) );
				
				doc.put("Message", errorMessage);
			} else {
				doc.put( "Message", pmx.getLocalizedMessage() );
			}
...
// This function will be called while deleting the Part.
        ...
        throws Exception
    {
      ...
      try
      {
        ...
        if(isMBOMInstalled){
        	String TYPE_ChangeAction=PropertyUtil.getSchemaProperty(context,"type_ChangeAction");
        	String relationshipManufacturingResponsibilityChange = PropertyUtil.getSchemaProperty(context, "relationship_ManufacturingResponsibilityChange");
        	String relationshipManufacturingResponsibility = PropertyUtil.getSchemaProperty(context, "relationship_ManufacturingResponsibility");


        	MapList mpList = partObj.getRelatedObjects(context,
        			relationshipManufacturingResponsibilityChange,
        			TYPE_ChangeAction+","+EngineeringConstants.TYPE_MECO+","+EngineeringConstants.TYPE_ECO,
        			null, new StringList(DomainRelationship.SELECT_ID),
        			true, false, (short)1,
        			null, null,0);
     

        	MapList mpListMR = partObj.getRelatedObjects(context,
        			relationshipManufacturingResponsibility,
        			EngineeringConstants.TYPE_PLANT,
        			null, new StringList(DomainRelationship.SELECT_ID),
        			true, false, (short)1,
        			null, null,0);
       
        	mpList.addAll(mpListMR);
        	...
        	if(mpList.size()>0 && mpList !=null)
        	{
        		...
        		while(itr.hasNext()) {
        			mpListMap = (Map) itr.next();
        			String relId=(String)mpListMap.get(DomainRelationship.SELECT_ID);
        			DomainRelationship.disconnect(context, relId);
        		}
        	}
        }
 ...
 /**
                                            ...
                                            boolean useAutoName) throws FrameworkException, MatrixException, Exception
     {
       ...
       if(policyFlag)
           {
           //IR-033556 - Starts
            childId=part.createPart(context, objectType, objectName, objectRev, PropertyUtil.getSchemaProperty(context, objectPolicy), context.getVault().getName(), useAutoName);
            //IR-033556 - Ends
           }
           else
           {
              //IR-033556 - Starts
              childId=part.createPart(context, objectType, objectName, objectRev, objectPolicy, context.getVault().getName(), useAutoName);
              //IR-033556 - Starts
           }
            //IR-033556 - Starts
        	...
        	if(!"TRUE".equalsIgnoreCase(objectID_Replaced) && partDtlMap.containsKey(selectProdctId)) // if object id is replaced as part of any EBOM authoring, we should not consider the data from cache
        	{
    			...
    			if(UIUtil.isNotNullAndNotEmpty(sPartId)){    			    			
    	        	...
    	        	if (productInfo.size() > 1) {
    	            	Iterator keySetIterator = productInfo.keySet().iterator();
    	            	String key;
    	            	prodIdKey = "";
    	            	prodAttrKey = "";
    	            	while (keySetIterator.hasNext()){
...
public MapList getEBOMsWithEffectivityRelSelectables (Context context, String[] args) throws Exception
{
	...
	/* Added for PUE REPORTS  */
		...
		if ("true".equals((String) paramMap.get("BOMCompare"))) {
			...
			String sPCFilterExpr1=null;
			com_bin1 = (UIUtil.isNotNullAndNotEmpty(pcFilter1)) ? PartMgtKernel.getInfo(context, pcFilter1, "attribute["+PartMgtUtil.getActualSchemaName(context, "attribute_FilterCompiledForm")+"]") : getCompiledBinaryCode(context, effect1,pcFilter1);
            com_bin2 = (UIUtil.isNotNullAndNotEmpty(pcFilter2)) ? PartMgtKernel.getInfo(context, pcFilter2, "attribute["+PartMgtUtil.getActualSchemaName(context, "attribute_FilterCompiledForm")+"]") : getCompiledBinaryCode(context, effect2,pcFilter2);
		}
	...
	public Map createPartJPO(Context context, String[] args) throws Exception {
		...
		try {
			...
			strPartId = part.createPartAndConnectRDO(context, sType, sName,
					sCustomRevisionLevel, sPolicy, context.getVault().getName(), sOwner,
    ...
    */
	...
	public HashMap partEditPostProcess(Context context, String[] args) throws Exception
	{
		...
		try{
			...
			if ( UIUtil.isNullOrEmpty(workUnderOID) ) {
				workUnderOID = (String) requestMap.get("selectedWorkUnder");
			}
			
			if ( UIUtil.isNotNullAndNotEmpty(workUnderOID) ) {
				ECMUtil.setWorkUnderChange(context, objectId, workUnderOID);
			}
			...
			if ( UIUtil.isNotNullAndNotEmpty(workUnderOID) ) {
				...
				iUserFactChangeLogService.updateObjectUserFact(context, new StringList(objectId), IUserFactChangeLogService.USER_EVENT_OBJECT_MODIFY);
			}
		
		/* Below code is to update the user facts on the change logs - End */
		}catch (Exception e){
			resultMap.put("Action", "ERROR");
    		resultMap.put("Message", e.toString().trim());
		}	
		return resultMap;
	}
...
throws Exception { //IR:073691
	...
	public MapList getPartWhereUsed(Context context, String[] args) throws Exception {
		...
		if (isValidData(objectId)) {
			...
			if(isWhereUsedPLBOM){
				...
				relSelect.add(DomainConstants.SELECT_ATTRIBUTE_UNITOFMEASURE);
			 ...
			 pcFilter = getStringValue(programMap, "PUEUEBOMProductConfigurationFilter_actualValue");			 
			  ...
			  objectSelect = createStringList(new String[] {DomainConstants.SELECT_ID, SELECT_PART_MODE, SELECT_RAISED_AGAINST_ECR,
				...
				if(UIUtil.isNotNullAndNotEmpty(pcFilter)){
					//strCompiledBinaryCode=new DomainObject(pcFilter).getInfo(context, "attribute["+PropertyUtil.getSchemaProperty(context, "attribute_FilterCompiledForm")+"]");
					strCompiledBinaryCode=PartMgtKernel.getInfo(context, pcFilter, "attribute["+PartMgtUtil.getActualSchemaName(context, "attribute_FilterCompiledForm")+"]");
					if(UIUtil.isNotNullAndNotEmpty(strEffectivityOID)){
						PartMgtKernel.displayMQLWarningMessage(context, PartMgtUtil.getDisplayPropertyValue(context,"emxUnresolvedEBOM.Alert.MultipleFilterSelected","emxUnresolvedEBOMStringResource"));
	        			   //${CLASS:emxContextUtil}.mqlNotice(context,"warning :"+EnoviaResourceBundle.getProperty(context,"emxUnresolvedEBOMStringResource",context.getLocale(),"emxUnresolvedEBOM.Alert.MultipleFilterSelected"));
					}
				}else				
	...
	public String getCompiledBinaryCode(Context context, String effectivityOID, String pcObjectId) throws Exception {
		if (isValidData(pcObjectId)) {
			Class klass = Class.forName("com.matrixone.apps.unresolvedebom.CFFUtil");
	...
	public StringList getUOMForPartWhereUsed(Context context, String[] args) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			strTemp = (String) ((Map) list.get(i)).get(DomainConstants.SELECT_ATTRIBUTE_UNITOFMEASURE);
			
			if (!isValidData(strTemp)) {
				String connectionId = (String) ((Map) list.get(i)).get(DomainConstants.SELECT_RELATIONSHIP_ID);
				if (isValidData(connectionId)) {
					strTemp = new DomainRelationship(connectionId).getAttributeValue(context, DomainConstants.ATTRIBUTE_UNIT_OF_MEASURE);
				} else {
					strTemp = "";
				}
			}
	...
	public StringList getRelNameForPartWhereUsed(Context context, String[] args) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			strTemp = (String) ((Map) list.get(i)).get("relationship");
			
			if (!isValidData(strTemp)) {
				String connectionId = (String) ((Map) list.get(i)).get(DomainConstants.SELECT_RELATIONSHIP_ID);
				
				if (isValidData(connectionId)) {
					strTemp = MqlUtil.mqlCommand(context, "print connection $1 select $2 dump", connectionId, "type");
				}
			}
			
			if (isValidData(strTemp)) {
				listReturn.add(UINavigatorUtil.getAdminI18NString("Relationship", strTemp, context.getSession().getLanguage()));
			}
			else {				
				listReturn.addElement("");
			}
	...
	public Map revisePartJPO(Context context, String[] args) throws Exception {
		...
		String workUnderOID = (String) programMap.get("WorkUnderOID");

		Map returnMap = new HashMap();

		Part part = new Part(strPartId);
		
		/* update First level BOM to Change Logs for Change Based sync - Start */
		...
		if(UIUtil.isNotNullAndNotEmpty(workUnderOID)){
			...
			iChangeActionServices.setChangeOnTransaction(context, workUnderOID);
		}
		DomainObject nextRev = new DomainObject(part.revisePart(context, sCustomRevisionLevel, sVault, true, false));
		
		BOMMgtUtil.updateRealtionshipUserFact_Revise(context, nextRev.getObjectId(context), strPartId);
		
		
		/* update First level BOM to Change Logs for Change Based sync - END*/		
	...
	public MapList getMyPartsForWidget(Context context, String selType,String functionality,StringList excludeTypes)throws Exception {
		...
		String whereExpression = (String)EngineeringUtil.getWhereExpressionForPnO(context,usercontext);
        ...
        for(int i=0; i<mlCommonBOMList.size(); i++){
            ...
            if(null != oEndItem && !"null".equals(oEndItem)) {
                ...
                if(oEndItem instanceof String) {
                    slEndItem.addElement((String) oEndItem);
                    slEndItemRel.addElement((String) oEIrelId);
                } else {
 ...
 */
   ...
   /**
    ...
    public void postProcessForClonePart(Context context, String[] args) throws Exception {
    	...
    	String workUnderOID = (String) requestMap.get("WorkUnderOID");
    	String isVPMVisible = (String) requestMap.get("isVPMVisible");
        ...
        while(st.hasMoreTokens()){
            ...
            if(!"true".equals(isVPMVisible)){
            	domNewObj.setAttributeValue(context, EngineeringConstants.ATTRIBUTE_VPM_VISIBLE, "FALSE");
			}
	        ...
	        /* update First level BOM to Change Logs for Change Based sync - Start */		        
	        BOMMgtUtil.updateRealtionshipUserFact_Clone(context, newObjectId, parentId, boolEBOMRelSelected, null);
			/* update First level BOM to Change Logs for Change Based sync - END*/
	        ...
	        /* Call collaborate service .st*/ 
	        
	        //Since the clone operation also does the object edit. We need to collaborate Clone and Edit operation
	        
	        ArrayList<String> list = new ArrayList<String>();
	        list.add(newObjectId);
	        JPO.invoke(context, "enoPartCollaboration", null, "collaboratePartCloneAndEdit", JPO.packArgs(list), void.class);	        
	    	
	        /* Call collaborate service .en*/
    ...
    public Boolean show3DToggleCommand(Context context, String[] args) throws Exception {
    	String s3DPlayshow = EnoviaResourceBundle.getProperty(context,"emxEngineeringCentral.Toggle.3DViewer");
 ...
 */
  ...
  /**
	...
	public int ensureECOConnected(Context context, String[] args) throws Exception {
		...
		if ( "true".equals(skipTriggerCheck) ) { return 0; }

		String sObjectId = getId(context);
			
		IPartLifecycleValidator iPartLifecycleValidator = IPartLifecycleValidator.getService(context, sObjectId);
			
		try {
			iPartLifecycleValidator.validateActiveChangeConnected(context);
		}
		catch (PartMgtException pmx) {
			emxContextUtil_mxJPO.mqlNotice( context, pmx.getLocalizedMessage() );
			return 1;
		}
...
public HashMap getDefaultRevisionOnPolicy (Context context,String[] args)throws Exception
	...
	*/
	...
	public Map updateUOMField(Context context, String[] args) throws Exception
	{
		...
		String createMode  = (String)requestMap.get("createMode"); 
		createMode = UIUtil.isNotNullAndNotEmpty(createMode)?createMode:"";
...
/*		Pattern relPattern = new Pattern(RELATIONSHIP_EBOM);
		...
		for(int i = 0; i < sAllRevisionStateWithEqual.size();i++){
			StringList slTempList = FrameworkUtil.split(((String)sAllRevisionStateWithEqual.get(i)), "=");
			String sStateValue= (slTempList.size()>1)?(String)slTempList.get(1):(String)slTempList.get(0);
			sStateValue = StringUtil.Replace(sStateValue, " ", "");
			sAllRevisionState.add(sStateValue);
		}
		if(sAllRevisionState != null && (sAllRevisionState.contains(DomainConstants.STATE_PART_RELEASE) || sAllRevisionState.contains(EngineeringConstants.STATE_COMPLETE)))
		...
		public Vector getPhysicalIdPath(Context context, String[] args) throws Exception {
			...
			while(mapListItr.hasNext())
			{
				...
				if("true".equalsIgnoreCase(sRootNode))
				{
					...
					String rootprdphysicalID=(String) partMap.get(EngineeringUtil.getValueForKey(partMap, "].physicalid"));
					...
					sProductPhyID=(String) partMap.get(EngineeringUtil.getValueForKey(partMap, "].physicalid"));	
...
public static Boolean isEditAllowedInFilteredView(Context context,String[] args) throws Exception
		...
		public int notifyNewOwner(Context context, String[] args)throws Exception {
			...
			if("owner".equalsIgnoreCase(kindOfOwner)){
				...
				Map partInfo = domObj.getInfo(context, objectSelects);			

				String strOwner = (String) partInfo.get(DomainConstants.SELECT_OWNER);
				//String newOwner = args[2];
				
				String[] inputArgs = new String[8];
				inputArgs[0]= strOwner;
    ...
    /**
     * Column Program to Display Design Name column in the BOM powerview. 
     * It displays the name of collaborated Product with hyperlink to Product Properties Page.
     * For non collaborated Parts, this column will be blank.
     * @param context the Context object.
     * @param args conatins the programMap
     * @return the Vector containing the href(for collaborated Parts)/Product Name(If it is invoked for Report)/Empty String (Non collaborated Parts)
     * @throws Exception
     */
    public Vector displayDesignNameInTable(Context context, String[] args) throws Exception
    {
    	Vector vNameVector = new Vector();
        HashMap programMap = (HashMap)JPO.unpackArgs(args);
        MapList objectList = (MapList)programMap.get("objectList");
        ...
        Map partDtlMap = (Map) objectList.get(0);    
        
		String prodId = "";
		String prodName = "";
		String prodAttrUsage = "";
		String sRootNode  = (String)partDtlMap.get("Root Node");

    	if(sRootNode!=null && "true".equalsIgnoreCase(sRootNode)) {            
    		StringList productDetailsList = getProductDetails(context, (String)partDtlMap.get(EngineeringConstants.SELECT_ID), null);
    		
    		if (productDetailsList != null && productDetailsList.size() > 2) {
    			prodId = productDetailsList.elementAt(0);
    			prodName = productDetailsList.elementAt(1);
    			prodAttrUsage = productDetailsList.elementAt(2);
    		}
    		            
    		if(UIUtil.isNotNullAndNotEmpty(prodId))
    			prodName = getDesignNameHref(context, prodName, prodId, prodAttrUsage, reportFormat);    
    		
			vNameVector.add(prodName);
    	}
    	else {
    		Iterator mapListItr = objectList.iterator();
            ...
            while(mapListItr.hasNext()) {
            	...
            	String objectID_Replaced = (String)partDtlMap.get("OBJECTID_REPLACED"); // if object id is replaced as part of any EBOM authoring, need to get latest info of that using db call 
            	boolean isAddOperation = !"true".equalsIgnoreCase((String) partDtlMap.get(EngineeringConstants.INITIAL_LOAD));
            	
            	StringList productDetailsList = new StringList();
            	
            	if(isAddOperation || "TRUE".equalsIgnoreCase(objectID_Replaced))
            		productDetailsList = getProductDetails(context, (String)partDtlMap.get(EngineeringConstants.SELECT_ID), null); 
            	else
            		productDetailsList = getProductDetails(context, (String)partDtlMap.get(EngineeringConstants.SELECT_ID), partDtlMap);
    ...
    /**
     * This method is for framing the href to the Product Properties page. 
     * @param context the Context object.
     * @param productName String conating the Name of the Product.
     * @param productId String conating the ID of the Product.
     * @param productAttrUsage String conating the value of the Usage Attribute of the Product.
     * @param reportFormat String containing the Report Format. For non report(non Export) use case, it will be null.
     * @return String containing the Product Name if the use case is related to report.
     * 		   String containing href to the Product Properties page.  
     * @throws Exception
     */
    private String getDesignNameHref(Context context,String productName, String productId, String productAttrUsage, String reportFormat) throws Exception 
    {
    	String strIcon = "3DPart".equalsIgnoreCase(productAttrUsage) ? "../common/images/I_3DPart.png" : "../common/images/I_VPMNavProduct.png"; 
		String modifyURL =  "../common/emxTree.jsp?objectId=" + productId ;
		
		if(UIUtil.isNotNullAndNotEmpty(productName)) 
			productName = productName.replace("&", "&amp;");

    	String href = "";
    	
    	if(UIUtil.isNullOrEmpty(reportFormat))
    		href = "<a href=\"JavaScript:emxTableColumnLinkClick('"+modifyURL+"', '700', '600', 'false', 'popup', '')\"><img src=\""+strIcon+"\" border=\"0\"/>"+productName+"</a>";
    	else
    		href = productName;
    ...
    /**
     * Field Program to display the collaborated Physical Porduct with hyperlink to Product Properties Page. 
     * @param context the Context object.
     * @param args contains the programMap
     * @return String containing the Product Name if the use case is related to report.
     * 		   String containing href to the Product Properties page.
     * 		   Empty String for non collaborated Parts.	
     * @throws Exception
     */
    public String displayDesignNameInForm(Context context, String[] args) throws Exception
    {
    	HashMap programMap = (HashMap) JPO.unpackArgs(args);
        Map requestMap     = (Map) programMap.get("requestMap");
        String objectId    = (String) requestMap.get("objectId");
    	...
    	if(UIUtil.isNotNullAndNotEmpty(objectId)){ 
    		...
    		StringList productDetailsList = getProductDetails(context, objectId, null);
    		...
    		if(UIUtil.isNotNullAndNotEmpty(prodId))
    			returnValue = getDesignNameHref(context, prodName, prodId, prodAttrUsage, reportFormat);
		}
    	return returnValue;
    }
    
    /**
     * Method to fetch the details of the Product collaborated with a Part.
     * If Data is available in the partDataMap, data is fetched from the Map.
     * If partDataMap is null, the data is fetched from Database with the Part ID.
     * @param context the Context object.
     * @param partId String conating the Part ID.
     * @param partDataMap Map containingParts Data.
     * @return StringList conating the Product ID, Product Name and value of the Attribute Usage.
     * @throws Exception
     */
    private StringList getProductDetails(Context context, String partId, Map partDataMap) throws Exception
    {
    	StringList returnList = new StringList();
    	StringList busSelects = new StringList();
		busSelects.add(SELECT_PRODUCT_ID);
		busSelects.add(SELECT_PRODUCT_NAME);
		busSelects.add(SELECT_PRODUCT_ATTR_USAGE);
		
		Map partDtlMap = null;
		
		if(null == partDataMap) {
			DomainObject dmRootObj = DomainObject.newInstance(context, partId);
			partDtlMap = dmRootObj.getInfo(context, busSelects);
		}
		else {
			partDtlMap = partDataMap;
		}
		
		if (partDtlMap != null && partDtlMap.size() > 1) {
        	Iterator keySetIterator = partDtlMap.keySet().iterator();
        	String key;
        	String prodIdKey = "";
        	String prodAttrUsageKey = "";
        	String prodNameKey = "";
        	...
        	while (keySetIterator.hasNext()){
        		...
        		if (key.indexOf("].id") > - 1) {
        			...
        			prodNameKey = key;
        		} else if (key.indexOf("V_usage]") > - 1) {
        			prodAttrUsageKey = key;
        		}
        	}
        	
        	if(UIUtil.isNotNullAndNotEmpty(prodIdKey))
        		returnList.add((String)partDtlMap.get(prodIdKey));
        	if(UIUtil.isNotNullAndNotEmpty(prodNameKey))
        		returnList.add((String)partDtlMap.get(prodNameKey));
        	if(UIUtil.isNotNullAndNotEmpty(prodAttrUsageKey))
        		returnList.add((String)partDtlMap.get(prodAttrUsageKey));
		}
