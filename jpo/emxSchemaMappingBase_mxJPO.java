...
public class emxSchemaMappingBase_mxJPO
{
        ...
        throws Exception
    {
		...
		if (UIUtil.isNotNullAndNotEmpty(sTempPath)  && iniFileName.contains("/Modules/") && !iniFileName.contains("X3D_TMP_DIR_INSTALL")) {
			iniFileName = sTempPath + iniFileName.substring(iniFileName.lastIndexOf("/Modules/")+8,iniFileName.length());
			System.out.println("iniFileName------------->>-- "+iniFileName);
		}
        // Get ordered schema names
        ArrayList lSchemaNames = new ArrayList(getOrderedSchemaElements(context, customMapping).values());
