...
//			String isProjectSpace = (String)taskInfo.get(SELECT_IS_KINDOF_PROJECT_SPACE);
			 ...
			 try {
					ContextUtil.pushContext(context);
					bwsl = BusinessObject.getSelectBusinessObjectData(context, new String[]{toId,fromId}, slBusSelect);
				}  finally {
					ContextUtil.popContext(context);
				}
			 
			 BusinessObjectWithSelect toBwsl = bwsl.get(0);
			 BusinessObjectWithSelect fromBwsl = bwsl.get(1);
			 
			 String isProjectSpace = toBwsl.getSelectData(SELECT_IS_KINDOF_PROJECT_SPACE);//(String)taskInfo.get(SELECT_IS_KINDOF_PROJECT_SPACE);
...
//				Map parentInfo = fromObj.getInfo(context, slBusSelect);
				String parentCS = fromBwsl.getSelectData(SELECT_COLLABORATIVE_SPACE);//(String) parentInfo.get(SELECT_COLLABORATIVE_SPACE);
				String parentOrg = fromBwsl.getSelectData(DomainConstants.SELECT_ORGANIZATION);//(String) parentInfo.get(DomainConstants.SELECT_ORGANIZATION);

				//Get to-side object details.
				String childCS = toBwsl.getSelectData(SELECT_COLLABORATIVE_SPACE);//(String) taskInfo.get(SELECT_COLLABORATIVE_SPACE);
				String childOrg = toBwsl.getSelectData(DomainConstants.SELECT_ORGANIZATION);//(String) taskInfo.get(DomainConstants.SELECT_ORGANIZATION);
